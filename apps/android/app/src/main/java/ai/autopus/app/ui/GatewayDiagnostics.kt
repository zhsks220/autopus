...
internal fun autopusAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Autopus Android gateway connection failure.
    ...
    - tell me whether `autopus devices list` should show a pending pairing request
    - if more signal is needed, ask for `autopus qr --json`, `autopus devices list`, and `autopus nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${autopusAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Autopus gateway diagnostics", report))
