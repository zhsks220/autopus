...
private const val NOTIFICATION_CHANNEL_BASE_ID = "autopus.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Autopus Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Autopus Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Autopus Active")
      }
