...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.autopus.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Autopus", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Autopus gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Autopus.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "AutopusShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Autopus is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Autopus Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("autopus-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
