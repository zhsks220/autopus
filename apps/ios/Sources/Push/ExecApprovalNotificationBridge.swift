...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "autopus.exec-approval"
    static let reviewActionIdentifier = "autopus.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.autopusPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.autopusPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func autopusPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
