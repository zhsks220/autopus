...
final class MotionService: MotionServicing {
    func activities(params: AutopusMotionActivityParams) async throws -> AutopusMotionActivityPayload {
        ...
        let mapped: [AutopusMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return AutopusMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = AutopusPedometerPayload(
