...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__AUTOPUS_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<AutopusTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.autopus", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = AutopusTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> AutopusTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return AutopusTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> AutopusTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> AutopusTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return AutopusTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> AutopusTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = AutopusTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: AutopusTalkPTTStopPayload) {
            ...
            group.addTask { [runId] in
                ...
                for await evt in stream {
                    ...
                    guard let chatEvent = try? GatewayPayloadDecoding.decode(
                        payload,
                        as: AutopusChatEventPayload.self)
                    else {
                        continue
                    }
                    guard chatEvent.runId == runId else { continue }
                    if let text = AutopusChatEventText.assistantText(from: chatEvent) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: AutopusAgentEventPayload.self)
            else {
                continue
            }
