...
@MainActor
enum AutopusAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class AutopusAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.autopus.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.autopus.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.autopus.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? AutopusAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "autopus.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "autopus.watch.promptId"
    static let sessionKeyKey = "autopus.watch.sessionKey"
    static let actionPrimaryIDKey = "autopus.watch.action.primary.id"
    static let actionPrimaryLabelKey = "autopus.watch.action.primary.label"
    static let actionSecondaryIDKey = "autopus.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "autopus.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "autopus.watch.action.primary"
    static let actionSecondaryIdentifier = "autopus.watch.action.secondary"
    static let actionIdentifierPrefix = "autopus.watch.action."
    static let actionIDKeyPrefix = "autopus.watch.action.id."
    static let actionLabelKeyPrefix = "autopus.watch.action.label."
    static let categoryPrefix = "autopus.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: AutopusWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> AutopusWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AutopusWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Autopus" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [AutopusWatchAction]) -> [UNNotificationAction] {
...
@main
struct AutopusApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(AutopusAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        AutopusAppModelRegistry.appModel = appModel
...
extension AutopusApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Autopus: installing uncaught exception handler")
