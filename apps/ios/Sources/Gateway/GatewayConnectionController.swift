...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [AutopusCapability.canvas.rawValue, AutopusCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(AutopusCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(AutopusCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = AutopusLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(AutopusCapability.location.rawValue) }

        caps.append(AutopusCapability.device.rawValue)
        caps.append(AutopusCapability.talk.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(AutopusCapability.watch.rawValue)
        }
        caps.append(AutopusCapability.photos.rawValue)
        caps.append(AutopusCapability.contacts.rawValue)
        caps.append(AutopusCapability.calendar.rawValue)
        caps.append(AutopusCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(AutopusCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            AutopusScreenCommand.record.rawValue,
            AutopusSystemCommand.notify.rawValue,
            AutopusChatCommand.push.rawValue,
        ...
        if caps.contains(AutopusCapability.camera.rawValue) {
        ...
        if caps.contains(AutopusCapability.location.rawValue) {
            commands.append(AutopusLocationCommand.get.rawValue)
        }
        if caps.contains(AutopusCapability.device.rawValue) {
            commands.append(AutopusDeviceCommand.status.rawValue)
            commands.append(AutopusDeviceCommand.info.rawValue)
        }
        if caps.contains(AutopusCapability.watch.rawValue) {
            commands.append(AutopusWatchCommand.status.rawValue)
            commands.append(AutopusWatchCommand.notify.rawValue)
        }
        if caps.contains(AutopusCapability.photos.rawValue) {
            commands.append(AutopusPhotosCommand.latest.rawValue)
        }
        if caps.contains(AutopusCapability.contacts.rawValue) {
            commands.append(AutopusContactsCommand.search.rawValue)
            commands.append(AutopusContactsCommand.add.rawValue)
        }
        if caps.contains(AutopusCapability.calendar.rawValue) {
            commands.append(AutopusCalendarCommand.events.rawValue)
            commands.append(AutopusCalendarCommand.add.rawValue)
        }
        if caps.contains(AutopusCapability.reminders.rawValue) {
            commands.append(AutopusRemindersCommand.list.rawValue)
            commands.append(AutopusRemindersCommand.add.rawValue)
        }
        if caps.contains(AutopusCapability.motion.rawValue) {
            commands.append(AutopusMotionCommand.activity.rawValue)
            commands.append(AutopusMotionCommand.pedometer.rawValue)
        }
