...
struct IOSGatewayChatTransport: AutopusChatTransport {
    private static let logger = Logger(subsystem: "ai.autopus", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> AutopusChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(AutopusChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> AutopusChatHistoryPayload {
        ...
        return try JSONDecoder().decode(AutopusChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [AutopusChatAttachmentPayload]) async throws -> AutopusChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [AutopusChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(AutopusChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(AutopusGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AutopusGatewayHealthOK.self))?.ok ?? true
