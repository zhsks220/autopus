...
final class ContactsService: ContactsServicing {
    ...
    func search(params: AutopusContactsSearchParams) async throws -> AutopusContactsSearchPayload {
        ...
        return AutopusContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: AutopusContactsAddParams) async throws -> AutopusContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return AutopusContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return AutopusContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> AutopusContactPayload {
        AutopusContactPayload(
