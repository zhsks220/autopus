...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> AutopusChatHistoryPayload
{
    AutopusChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> AutopusChatSessionEntry {
...
private func thinkingOption(_ id: String, label: String? = nil) -> AutopusChatThinkingLevelOption {
    AutopusChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> AutopusChatSessionEntry
{
    AutopusChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> AutopusChatModelChoice {
    AutopusChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [AutopusChatHistoryPayload],
    sessionsResponses: [AutopusChatSessionsListResponse] = [],
    modelResponses: [[AutopusChatModelChoice]] = [],
    ...
    -> (TestChatTransport, AutopusChatViewModel)
{
...
private func sendUserMessage(_ vm: AutopusChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, AutopusChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [AutopusChatHistoryPayload]
    private let sessionsResponses: [AutopusChatSessionsListResponse]
    private let modelResponses: [[AutopusChatModelChoice]]
    ...
    private let stream: AsyncStream<AutopusChatTransportEvent>
    private let continuation: AsyncStream<AutopusChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<AutopusChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> AutopusChatHistoryPayload {
        ...
        return self.historyResponses.last ?? AutopusChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [AutopusChatAttachmentPayload]) async throws -> AutopusChatSendResponse
    {
        ...
        return AutopusChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> AutopusChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? AutopusChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: AutopusChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func appendsExternalSessionUserMessageForActiveSession() async throws {
        ...
        transport.emit(
            .sessionMessage(
                AutopusSessionMessageEventPayload(
                    sessionKey: "agent:main:main",
                    message: AutopusChatMessage(
                        role: "user",
                        content: [
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = AutopusChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = AutopusChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: AutopusChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(AutopusChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == AutopusChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = AutopusChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: AutopusChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = AutopusChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = AutopusChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = AutopusChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func decodesGatewayThinkingMetadataFromSessionList() throws {
        ...
        let decoded = try JSONDecoder().decode(
            AutopusChatSessionsListResponse.self,
            from: Data(json.utf8))
...
"""]],
        ...
        let vm = await MainActor.run { AutopusChatViewModel(sessionKey: "main", transport: transport) }
