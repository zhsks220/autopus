...
struct ChatMessageBubble: View {
    let message: AutopusChatMessage
    let style: AutopusChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? AutopusChatTheme.userText : AutopusChatTheme.assistantText
    ...
    private var inlineAttachments: [AutopusChatMessageContent] {
    ...
    private var toolCalls: [AutopusChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [AutopusChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? AutopusChatTheme.userBubble
        }
        if self.style == .onboarding {
            return AutopusChatTheme.onboardingAssistantBubble
        }
        return AutopusChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return AutopusChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: AutopusChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? AutopusChatTheme.userText : AutopusChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: AutopusChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(AutopusChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [AutopusChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: AutopusChatTheme.assistantText)
            }
        }
    }
}
