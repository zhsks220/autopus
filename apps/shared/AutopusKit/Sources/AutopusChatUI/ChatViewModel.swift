...
private let chatUILogger = Logger(subsystem: "ai.autopus", category: "AutopusChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class AutopusChatViewModel {
    public static let defaultModelSelectionID = "__default__"
    private static let maxAttachmentBytes = 5_000_000

    public private(set) var messages: [AutopusChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var thinkingLevelOptions: [AutopusChatThinkingLevelOption]
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [AutopusChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [AutopusPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [AutopusChatPendingToolCall] = []
    public private(set) var sessions: [AutopusChatSessionEntry] = []
    private let transport: any AutopusChatTransport
    private var sessionDefaults: AutopusChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: AutopusChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any AutopusChatTransport,
    ...
    public var sessionChoices: [AutopusChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [AutopusChatSessionEntry] = []
    ...
    private static let baseThinkingLevelOptions: [AutopusChatThinkingLevelOption] = [
        AutopusChatThinkingLevelOption(id: "off", label: "off"),
        AutopusChatThinkingLevelOption(id: "minimal", label: "minimal"),
        AutopusChatThinkingLevelOption(id: "low", label: "low"),
        AutopusChatThinkingLevelOption(id: "medium", label: "medium"),
        AutopusChatThinkingLevelOption(id: "high", label: "high"),
    ]
    ...
    public func removeAttachment(_ id: AutopusPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [AutopusChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: AutopusChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: AutopusChatMessage) -> AutopusChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> AutopusChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return AutopusChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: AutopusChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: AutopusChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: AutopusChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [AutopusChatMessage],
        incoming: [AutopusChatMessage]) -> [AutopusChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [AutopusChatMessage]) -> [AutopusChatMessage] {
        var result: [AutopusChatMessage] = []
    ...
    private static func dedupeKey(for message: AutopusChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [AutopusChatMessageContent] = [
            AutopusChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> AutopusChatAttachmentPayload in
            AutopusChatAttachmentPayload(
                type: att.type,
    ...
    private func resolvedThinkingLevelOptions(
        for currentSession: AutopusChatSessionEntry?) -> [AutopusChatThinkingLevelOption]
    {
    ...
    private static func sessionModelMatchesDefaults(
        _ session: AutopusChatSessionEntry,
        defaults: AutopusChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [AutopusChatThinkingLevelOption]?) -> [AutopusChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return AutopusChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [AutopusChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return AutopusChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    private static func withCurrentThinkingOption(
        _ options: [AutopusChatThinkingLevelOption],
        current: String) -> [AutopusChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [AutopusChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [AutopusChatThinkingLevelOption]) -> [AutopusChatThinkingLevelOption]
    {
        var result: [AutopusChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    private func placeholderSession(key: String) -> AutopusChatSessionEntry {
        AutopusChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func updateCurrentSessionThinkingLevel(_ thinkingLevel: String?, sessionKey: String) {
        ...
        self.sessions[index] = AutopusChatSessionEntry(
            key: current.key,
            kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: AutopusChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleSessionMessageEvent(_ payload: AutopusSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: AutopusChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: AutopusAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = AutopusChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> AutopusPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
