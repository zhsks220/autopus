...
@MainActor
public struct AutopusChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: AutopusChatViewModel
    ...
    public init(
        viewModel: AutopusChatViewModel,
    ...
    private var visibleMessages: [AutopusChatMessage] {
        let base: [AutopusChatMessage]
    ...
    private func mergeToolResults(in messages: [AutopusChatMessage]) -> [AutopusChatMessage] {
    ...
    private func isToolResultMessage(_ message: AutopusChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: AutopusChatMessage) -> Bool {
    ...
    private func primaryText(in message: AutopusChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: AutopusChatMessage) -> Bool {
    ...
    private func toolCalls(in message: AutopusChatMessage) -> [AutopusChatMessageContent] {
    ...
    private func inlineToolResults(in message: AutopusChatMessage) -> [AutopusChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: AutopusChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: AutopusChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(AutopusChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
