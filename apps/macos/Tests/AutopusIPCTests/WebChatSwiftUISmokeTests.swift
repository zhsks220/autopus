...
struct WebChatSwiftUISmokeTests {
    private struct TestTransport: AutopusChatTransport {
        func requestHistory(sessionKey: String) async throws -> AutopusChatHistoryPayload {
            let json = """
            {"sessionKey":"\(sessionKey)","sessionId":null,"messages":[],"thinkingLevel":"off"}
            """
            return try JSONDecoder().decode(AutopusChatHistoryPayload.self, from: Data(json.utf8))
        }
            ...
            attachments _: [AutopusChatAttachmentPayload]) async throws -> AutopusChatSendResponse
        {
            ...
            """
            return try JSONDecoder().decode(AutopusChatSendResponse.self, from: Data(json.utf8))
        }
