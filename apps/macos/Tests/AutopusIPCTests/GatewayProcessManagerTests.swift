...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["AUTOPUS_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "autopus-gateway",
                executablePath: "/tmp/autopus-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 autopus-gateway @ /tmp/autopus-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error
