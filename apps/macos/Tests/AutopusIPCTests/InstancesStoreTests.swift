...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: AutopusProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: AutopusProtocol.AnyCodable] = [
            "presence": .init([AutopusProtocol.AnyCodable(entry)]),
        ]
        let payload = AutopusProtocol.AnyCodable(payloadMap)
