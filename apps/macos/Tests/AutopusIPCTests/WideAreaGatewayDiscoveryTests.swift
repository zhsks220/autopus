...
import Foundation
import Testing
@testable import AutopusDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("AUTOPUS_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("AUTOPUS_WIDE_AREA_DOMAIN", "autopus.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("AUTOPUS_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("AUTOPUS_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._autopus-gw._tcp.autopus.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.autopus.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (Autopus)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/autopus/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Autopus)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/autopus/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._autopus-gw._tcp.autopus.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
