...
struct ConnectOptions {
    ...
    var clientId: String = "autopus-macos"
...
func runConnect(_ args: [String]) async {
    ...
    if opts.help {
        ...
        Usage:
          autopus-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
        ...
        Options:
          ...
          --json             Emit JSON
          --client-id <id>   Override client id (default: autopus-macos)
    ...
    do {
        ...
        let displayName = opts.displayName ?? Host.current().localizedName ?? "Autopus macOS Debug CLI"
...
private func printConnectOutput(_ output: ConnectOutput, json: Bool) {
    ...
    print("Autopus macOS Gateway Connect")
