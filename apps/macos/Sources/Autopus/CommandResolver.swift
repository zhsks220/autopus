...
enum CommandResolver {
    private static let projectRootDefaultsKey = "autopus.gatewayProjectRootPath"
    private static let helperName = "autopus"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let autopusEntry = root.appendingPathComponent("autopus.mjs").path
        if FileManager().isReadableFile(atPath: autopusEntry) { return autopusEntry }
        let binEntry = root.appendingPathComponent("bin/autopus.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/autopus")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let autopusPaths = self.autopusManagedPaths(home: home)
        if !autopusPaths.isEmpty {
            extras.insert(contentsOf: autopusPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + autopusPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func autopusManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".autopus"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func autopusExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectAutopusExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("autopus.mjs").path,
            root.appendingPathComponent("bin/autopus.js").path,
        ]
    ...
    static func hasAnyAutopusInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.autopusExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func autopusNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let autopusPath = self.projectAutopusExecutable(projectRoot: root) {
            return [autopusPath, subcommand] + extraArgs
        }
        if let autopusPath = self.autopusExecutable(searchPaths: searchPaths) {
            return [autopusPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "autopus", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            autopus entrypoint missing (looked for dist/index.js or autopus.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real autopus CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/autopus"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/autopus.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/autopus.mjs"
            node "$PRJ/autopus.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/autopus.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/autopus.js"
            node "$PRJ/bin/autopus.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent autopus"
          pnpm --silent autopus \(quotedArgs);
        else
          echo "autopus CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? AutopusConfigFile.loadDict()
