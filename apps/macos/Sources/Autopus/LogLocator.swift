...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["AUTOPUS_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/autopus")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("autopus-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("autopus-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/autopus/ (rolling or stdout), or nil if none exist.
