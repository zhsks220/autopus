...
final class AppState {
    private static let logger = Logger(subsystem: "ai.autopus", category: "app-state")
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            AutopusConfigFile.canonicalHostForComparison(host) ==
                AutopusConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = AutopusConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: AutopusConfigFile.loadDict(),
            draft: .init(
        ...
        guard AutopusConfigFile.saveDict(synced.root) else {
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/autopus"
        state.remoteCliPath = ""
        return state
    }
}
