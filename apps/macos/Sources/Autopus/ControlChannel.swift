...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: AutopusProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.autopus", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AutopusKit.AnyCodable]()) {
                $0[$1.key] = AutopusKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AutopusProtocol.AnyCodable?) -> [String: AutopusProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AutopusProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AutopusKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AutopusProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("autopus.control.heartbeat")
    static let controlAgentEvent = Notification.Name("autopus.control.agent")
}
