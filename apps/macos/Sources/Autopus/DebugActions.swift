...
enum DebugActions {
    private static let verboseDefaultsKey = "autopus.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "autopus.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = AutopusPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Autopus", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = AutopusPaths.configURL
        guard
