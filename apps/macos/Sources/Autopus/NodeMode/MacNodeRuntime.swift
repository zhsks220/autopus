...
actor MacNodeRuntime {
    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            ...
            case AutopusBrowserCommand.proxy.rawValue:
            ...
            case AutopusLocationCommand.get.rawValue:
            ...
            case AutopusSystemCommand.run.rawValue:
                return try await self.handleSystemRun(req)
            case AutopusSystemCommand.which.rawValue:
                return try await self.handleSystemWhich(req)
            case AutopusSystemCommand.notify.rawValue:
                return try await self.handleSystemNotify(req)
            case AutopusSystemCommand.execApprovalsGet.rawValue:
                return try await self.handleSystemExecApprovalsGet(req)
            case AutopusSystemCommand.execApprovalsSet.rawValue:
    ...
    private func handleA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case AutopusCanvasA2UICommand.push.rawValue,
             AutopusCanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleBrowserProxyInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        guard AutopusConfigFile.browserControlEnabled() else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: AutopusNodeError(
    ...
    private static func resolveA2UIHostUrl(from raw: String?) -> String? {
        ...
        return baseUrl.appendingPathComponent("__autopus__/a2ui/").absoluteString + "?platform=macos"
    }
    ...
    private func handleSystemRun(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(AutopusSystemRunParams.self, from: req.paramsJSON)
    ...
    private func handleSystemWhich(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(AutopusSystemWhichParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> AutopusLocationMode {
        ...
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: AutopusNodeError(code: code, message: message))
    }

    private static func encodeCanvasSnapshot(
        image: NSImage,
        format: AutopusCanvasSnapshotFormat,
        maxWidth: Int?,
