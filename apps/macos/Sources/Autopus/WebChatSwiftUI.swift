...
private let webChatSwiftLogger = Logger(subsystem: "ai.autopus", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "autopus.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: AutopusChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> AutopusChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(AutopusChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            AutopusChatSessionsDefaults(
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return AutopusChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> AutopusChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                AutopusGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "session.message":
                ...
                guard let message = try? JSONDecoder().decode(
                    AutopusSessionMessageEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: AutopusProtocol.ModelChoice) -> AutopusChatModelChoice {
        AutopusChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<AutopusChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any AutopusChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = AutopusChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: AutopusChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<AutopusChatView>) -> NSViewController
    {
