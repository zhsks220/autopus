...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.autopus.mac`) even if the shipped
    ...
    {
        if environment["AUTOPUS_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "autopus.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "autopus.nixMode") { return true }
