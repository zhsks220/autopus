...
enum AutopusLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = AutopusOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = AutopusFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        AutopusLogging.bootstrapIfNeeded()
        let label = AutopusLogging.makeLabel(subsystem: subsystem, category: category)
...
struct AutopusOSLogHandler: AppLogLevelBackedHandler {
...
struct AutopusFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = AutopusLogging.parseLabel(self.label)
