...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.autopus", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> AutopusSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: AutopusSessionPreviewEntry,
    ...
    private static func cache(payload: AutopusSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: AutopusChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [AutopusKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [AutopusKit.AnyCodable]) -> [AutopusChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(AutopusChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: AutopusChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: AutopusChatMessage) -> Bool {
    ...
    private static func toolNames(for message: AutopusChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: AutopusChatMessage) -> String? {
