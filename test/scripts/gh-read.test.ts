...
describe("gh-read helpers", () => {
  it("finds repo from gh args", () => {
    expect(parseRepoArg(["pr", "view", "42", "-R", "autopus/autopus"])).toBe("autopus/autopus");
    expect(parseRepoArg(["run", "list", "--repo=autopus/docs"])).toBe("autopus/docs");
    expect(parseRepoArg(["pr", "view", "42"])).toBeNull();
  });

  it("normalizes repo strings from common git formats", () => {
    expect(normalizeRepo("autopus/autopus")).toBe("autopus/autopus");
    expect(normalizeRepo("github.com/autopus/autopus")).toBe("autopus/autopus");
    expect(normalizeRepo("https://github.com/autopus/autopus.git")).toBe("autopus/autopus");
    expect(normalizeRepo("git@github.com:autopus/autopus.git")).toBe("autopus/autopus");
