...
describe("install.sh", () => {
  ...
  it("loads nvm before checking Node.js so stale system Node does not win", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "autopus-install-nvm-"));
  ...
  it("promotes a supported Linux Node binary over stale PATH entries", () => {
    const tmp = mkdtempSync(join(tmpdir(), "autopus-install-node-promote-"));
  ...
  it("persists a supported Linux Node path before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "autopus-install-linux-node-path-"));
    const home = join(tmp, "home");
  ...
  it("warns before redirecting an unwritable npm prefix", () => {
    const tmp = mkdtempSync(join(tmpdir(), "autopus-install-npm-prefix-"));
    const home = join(tmp, "home");
    ...
    expect(result?.stdout).toContain("npm i -g autopus@latest");
  ...
  it("persists npm prefix PATH before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "autopus-install-npm-prefix-shell-"));
    const home = join(tmp, "home");
    mkdirSync(home, { recursive: true });
    writeFileSync(
      join(home, ".bashrc"),
  ...
  it("uses a quoted absolute autopus path in follow-up commands when npm bin is not on the original PATH", () => {
    const tmp = mkdtempSync(join(tmpdir(), "autopus-install-command-"));
    const npmBin = join(tmp, "npm bin");
    const visibleBin = join(tmp, "visible-bin");
    mkdirSync(npmBin, { recursive: true });
    mkdirSync(visibleBin, { recursive: true });
    const autopusBin = join(npmBin, "autopus");
    writeFileSync(autopusBin, "#!/bin/sh\nexit 0\n");
    chmodSync(autopusBin, 0o755);
    ...
    try {
      result = runInstallShell(`
        ...
        printf 'missing=%s\\n' "$(autopus_command_for_user "${autopusBin}")"
    ...
    expect(result?.status).toBe(0);
    expect(result?.stdout).toContain(`missing=${autopusBin.replace(/ /g, "\\ ")}`);
    expect(result?.stdout).toContain("present=autopus");
  });
...
describe("install.sh macOS Homebrew Node behavior", () => {
  ...
  it("reruns spinner-wrapped commands when gum reports ioctl failure", () => {
    const dir = mkdtempSync(join(tmpdir(), "autopus-install-sh-gum-"));
    try {
...
describe("install.sh duplicate Autopus install detection", () => {
  it("warns with concrete package paths and versions for duplicate npm roots", () => {
    const result = runInstallShell(`
      ...
      mkdir -p "$root/brew/autopus" "$root/fnm/autopus"
      printf '{"version":"2026.3.7"}\\n' > "$root/brew/autopus/package.json"
      printf '{"version":"2026.3.1"}\\n' > "$root/fnm/autopus/package.json"
      collect_autopus_npm_root_candidates() { printf '%s\\n' "$root/brew" "$root/fnm"; }
      AUTOPUS_BIN="$root/fnm/.bin/autopus"
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_autopus_global_installs
    `);

    expect(result.status).toBe(0);
    expect(result.stdout).toContain("Multiple Autopus global installs detected");
    expect(result.stdout).toContain("2026.3.7");
    expect(result.stdout).toContain("2026.3.1");
    expect(result.stdout).toContain("/brew/autopus");
    expect(result.stdout).toContain("/fnm/autopus");
    expect(result.stdout).toContain("Active autopus:");
    expect(result.stdout).toContain("npm uninstall -g autopus");
  });

  it("stays quiet when only one Autopus npm root exists", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      root="$(mktemp -d)"
      trap 'rm -rf "$root"' EXIT
      mkdir -p "$root/only/autopus"
      printf '{"version":"2026.3.7"}\\n' > "$root/only/autopus/package.json"
      collect_autopus_npm_root_candidates() { printf '%s\\n' "$root/only"; }
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_autopus_global_installs
    `);
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).not.toContain("Multiple Autopus global installs detected");
  });
});
