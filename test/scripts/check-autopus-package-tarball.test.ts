...
const CHECK_SCRIPT = "scripts/check-autopus-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "autopus-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "autopus", version }));
    writeFileSync(
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><autopus-app></autopus-app>",
    ...
    const tarball = join(root, "autopus.tgz");
...
describe("check-autopus-package-tarball", () => {
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("Autopus package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
