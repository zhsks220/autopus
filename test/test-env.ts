...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.AUTOPUS_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "AUTOPUS_TEST_FAST", value: process.env.AUTOPUS_TEST_FAST },
    {
      key: "AUTOPUS_STRICT_FAST_REPLY_CONFIG",
      value: process.env.AUTOPUS_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "AUTOPUS_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.AUTOPUS_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "AUTOPUS_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.AUTOPUS_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "AUTOPUS_STATE_DIR", value: process.env.AUTOPUS_STATE_DIR },
    { key: "AUTOPUS_CONFIG_PATH", value: process.env.AUTOPUS_CONFIG_PATH },
    { key: "AUTOPUS_GATEWAY_PORT", value: process.env.AUTOPUS_GATEWAY_PORT },
    { key: "AUTOPUS_BRIDGE_ENABLED", value: process.env.AUTOPUS_BRIDGE_ENABLED },
    { key: "AUTOPUS_BRIDGE_HOST", value: process.env.AUTOPUS_BRIDGE_HOST },
    { key: "AUTOPUS_BRIDGE_PORT", value: process.env.AUTOPUS_BRIDGE_PORT },
    { key: "AUTOPUS_CANVAS_HOST_PORT", value: process.env.AUTOPUS_CANVAS_HOST_PORT },
    { key: "AUTOPUS_TEST_HOME", value: process.env.AUTOPUS_TEST_HOME },
    { key: "AUTOPUS_AGENT_DIR", value: process.env.AUTOPUS_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "autopus-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.AUTOPUS_TEST_HOME = tempHome;
  process.env.AUTOPUS_TEST_FAST = "1";
  process.env.AUTOPUS_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.AUTOPUS_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.AUTOPUS_BRIDGE_ENABLED;
  delete process.env.AUTOPUS_BRIDGE_HOST;
  delete process.env.AUTOPUS_BRIDGE_PORT;
  delete process.env.AUTOPUS_CANVAS_HOST_PORT;
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.AUTOPUS_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.AUTOPUS_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".autopus");
  const priorIsolatedHome = params.env.AUTOPUS_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".autopus")
  ) {
    realStateDir = path.join(params.realHome, ".autopus");
  }
  const tempStateDir = path.join(params.tempHome, ".autopus");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.AUTOPUS_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.AUTOPUS_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "autopus.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "autopus.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    ...
    process.env.AUTOPUS_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.AUTOPUS_LIVE_USE_REAL_HOME);
