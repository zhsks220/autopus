...
export function resolveAutopusManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type AutopusManifestRequires = {
  bins: string[];
  anyBins: string[];
  env: string[];
  config: string[];
...
export function resolveAutopusManifestRequires(
  metadataObj: Record<string, unknown>,
): AutopusManifestRequires | undefined {
  const requiresRaw =
...
export function resolveAutopusManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export function resolveAutopusManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedAutopusManifestInstallBase = {
...
export function parseAutopusManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedAutopusManifestInstallBase | undefined {
  ...
  const spec: ParsedAutopusManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyAutopusManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedAutopusManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
