...
const fixtureRoot = path.join(process.cwd(), ".tmp", `autopus-hook-install-${randomUUID()}`);
...
const npmPackHooksBuffer = await createTarGzHookPackBuffer({
  packageName: "@autopus/test-hooks",
  hookName: "one-hook",
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@autopus/test-hooks",
      version: "0.0.1",
      autopus: { hooks: params.hooks },
...
async function createZipHookPackBuffer(params: {
  ...
  const packageJson = JSON.stringify({
    ...
    version: "0.0.1",
    autopus: { hooks: [`./hooks/${params.hookName}`] },
  });
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "autopus.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "autopus.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.npmResolution?.resolvedSpec).toBe("@autopus/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@autopus/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = runCommandWithTimeoutMock;
    mockNpmPackMetadataResult(run, {
      id: "@autopus/test-hooks@0.0.2-beta.1",
      name: "@autopus/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@autopus/test-hooks@beta"');
    }
  });
});
