...
export const AUTOPUS_WRAPPER_ENV_KEY = "AUTOPUS_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/autopus -> .pnpm/autopus@X.Y.Z/...
...
export async function resolveAutopusWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${AUTOPUS_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveAutopusWrapperPath(params.wrapperPath);
  if (wrapperPath) {
