...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when AUTOPUS_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".autopus", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when AUTOPUS_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", AUTOPUS_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".autopus-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers AUTOPUS_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        AUTOPUS_PROFILE: "rescue",
        AUTOPUS_STATE_DIR: "C:\\State\\autopus",
      },
      expected: path.join("C:\\State\\autopus", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", AUTOPUS_PROFILE: "default" },
      expected: path.join("/home/test", ".autopus", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "autopus-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Autopus Gateway",
          "cd /d C:\\Projects\\autopus",
          "set NODE_ENV=production",
          "set AUTOPUS_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\autopus",
          environment: {
            NODE_ENV: "production",
            AUTOPUS_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\autopus\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\autopus\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Autopus Share\\node.exe" "\\\\fileserver\\Autopus Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Autopus Share\\node.exe",
            "\\\\fileserver\\Autopus Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from AUTOPUS_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ AUTOPUS_STATE_DIR: path.join(tmpDir, "custom-state") }),
