...
function resolveSystemdServiceName(env: GatewayServiceEnv): string {
  const override = env.AUTOPUS_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
  }
  return resolveGatewaySystemdServiceName(env.AUTOPUS_PROFILE);
}
...
async function writeSystemdGatewayEnvironmentFile(params: {
  ...
  /** Autopus-managed keys that must not be preserved from an old env file; stale file values
  ...
  // Autopus-managed keys (identified by inlineManagedKeys) are excluded: a stale
