...
const resolveExistingKey = vi.hoisted(() =>
  vi.fn<(config: AutopusConfig, provider: string) => string | undefined>(() => undefined),
);
const hasExistingKey = vi.hoisted(() =>
  vi.fn<(config: AutopusConfig, provider: string) => boolean>(() => false),
);
...
const listConfiguredWebSearchProviders = vi.hoisted(() =>
  vi.fn<(params?: { config?: AutopusConfig }) => PluginWebSearchProviderEntry[]>(() => []),
);
...
function createEnabledFirecrawlSearchConfig(): AutopusConfig {
  return {
    tools: {
      web: {
        search: {
...
describe("finalizeSetupWizard", () => {
  ...
  it("resolves gateway password SecretRef for probe but omits auth from TUI hatch", async () => {
    ...
    process.env.AUTOPUS_GATEWAY_PASSWORD = "resolved-gateway-password"; // pragma: allowlist secret
  ...
  it("uses the resolved setup password for health checks", async () => {
    vi.stubEnv("AUTOPUS_GATEWAY_PASSWORD", "env-password");
