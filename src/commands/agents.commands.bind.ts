...
function resolveTargetAgentIdOrExit(params: {
  ...
  if (!agentId) {
    params.runtime.error(
      `Unable to resolve agent id. Run ${formatCliCommand("autopus agents list")} to choose one.`,
    );
...
export async function agentsBindingsCommand(
  ...
  if (filterAgentId && !hasAgent(cfg, filterAgentId)) {
    runtime.error(
      `Agent "${filterAgentId}" not found. Run ${formatCliCommand("autopus agents list")} to see configured agents.`,
    );
    runtime.exit(1);
    return;
  }
