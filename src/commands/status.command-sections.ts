...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("autopus security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("autopus security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusFooterLines(params: {
  ...
  return [
    ...
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("autopus status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("autopus logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("autopus node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("autopus status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("autopus gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`autopus devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("autopus devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("autopus devices list")}`),
  ];
}
