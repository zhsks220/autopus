...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Autopus", ".autopus");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/autopus-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".autopus");
  ...
  it("anchors cloud detection to OS homedir when AUTOPUS_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".autopus");
    const originalAutopusHome = process.env.AUTOPUS_HOME;
    process.env.AUTOPUS_HOME = "/tmp/autopus-home-override";
