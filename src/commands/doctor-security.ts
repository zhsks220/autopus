...
import type { AutopusConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: AutopusConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: AutopusConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "Autopus default (full)";
  const defaultRequestedAskSource = "Autopus default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("autopus approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: AutopusConfig): string[] {
  void cfg;
  return [];
}

function collectExecFilesystemPolicyWarnings(cfg: AutopusConfig): string[] {
...
export async function noteSecurityWarnings(cfg: AutopusConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("autopus security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.autopus/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("autopus approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("autopus configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("autopus config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("autopus doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("autopus config set gateway.bind loopback")}`,
