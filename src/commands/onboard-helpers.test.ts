...
describe("handleReset", () => {
  it("uses active profile paths for destructive reset targets", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "autopus-reset-profile-"));
    const profileStateDir = path.join(homeDir, ".autopus-work");
    const defaultStateDir = path.join(homeDir, ".autopus");
    const profileConfigPath = path.join(profileStateDir, "autopus.json");
    ...
    vi.stubEnv("AUTOPUS_HOME", homeDir);
    vi.stubEnv("AUTOPUS_PROFILE", "work");
    vi.stubEnv("AUTOPUS_STATE_DIR", profileStateDir);
    vi.stubEnv("AUTOPUS_CONFIG_PATH", profileConfigPath);
