...
const SEARCH_TOOL = "https://docs.autopus.ai/mcp.SearchAutopus";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.autopus.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('autopus docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.autopus.ai/");
      runtime.log(`Search: ${formatCliCommand('autopus docs "your query"')}`);
    }
    return;
  }
