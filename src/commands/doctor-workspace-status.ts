...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("autopus tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("autopus tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: AutopusConfig) {
