...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("autopus security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("autopus status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("autopus tasks maintenance --apply")}`,
