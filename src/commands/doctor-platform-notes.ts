...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".autopus", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: AutopusConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [["AUTOPUS_GATEWAY_TOKEN", await getenv("AUTOPUS_GATEWAY_TOKEN")]] as const;
  const passwordEntries = [
    ["AUTOPUS_GATEWAY_PASSWORD", await getenv("AUTOPUS_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "AUTOPUS_GATEWAY_PASSWORD"}\` is set; it can make local clients use a different password than gateway.auth.password.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = normalizeOptionalString(env.AUTOPUS_NO_RESPAWN) ?? "";
  const lines: string[] = [];
