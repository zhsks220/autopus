...
export const resolveAutopusPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadAutopusPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("autopus/plugin-sdk/provider-stream-shared", () => ({
...
beforeEach(() => {
  ...
  resolveAutopusPackageRoot.mockReset().mockResolvedValue(null);
  ...
  originalUpdateInProgress = process.env.AUTOPUS_UPDATE_IN_PROGRESS;
  process.env.AUTOPUS_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "autopus-doctor-state-"));
