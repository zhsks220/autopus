...
const createTempDir = () => tempDirs.make("autopus-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets AUTOPUS_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "autopus",
    });

    expect(env.AUTOPUS_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("autopus");
  });

  it("overrides pre-existing AUTOPUS_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      AUTOPUS_SHELL: "wrong",
    });
    expect(env.AUTOPUS_SHELL).toBe("acp-client");
  ...
  it("preserves AUTOPUS_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        AUTOPUS_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["AUTOPUS_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Autopus bridge", () => {
    ...
    expect(env.AUTOPUS_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.AUTOPUS_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "autopus",
        serverArgs: ["acp"],
        defaultServerCommand: "autopus",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("AUTOPUS_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "autopus", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "autopus", "dist", "entry.js");
    const shimPath = path.join(dir, "autopus.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\autopus\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/autopus-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/autopus-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/autopus-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
