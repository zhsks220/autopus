...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: AutopusConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/autopus" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: AutopusConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: AutopusConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: AutopusConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: AutopusConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: AutopusConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgDisable: AutopusConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaultsWithoutAgentModel: AutopusConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
    ...
    const inheritedCfg: AutopusConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: AutopusConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: AutopusConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/autopus");
  });

  it("uses AUTOPUS_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "autopus-home");
    vi.stubEnv("AUTOPUS_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as AutopusConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".autopus", "workspace"));
  });

  it("uses AUTOPUS_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "autopus-home");
    vi.stubEnv("AUTOPUS_HOME", home);
    // Clear state dir so it falls back to AUTOPUS_HOME
    vi.stubEnv("AUTOPUS_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as AutopusConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".autopus", "agents", "main", "agent"));
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/autopus-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/autopus-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "autopus-agent-scope-"));
