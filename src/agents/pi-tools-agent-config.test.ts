...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "autopus-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: AutopusConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<AutopusConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<AutopusConfig["agents"]>["list"]>[number]["tools"];
  }): AutopusConfig {
    return {
      tools: params.tools,
      agents: {
