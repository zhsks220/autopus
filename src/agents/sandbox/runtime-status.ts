...
function resolveMainSessionKeyForSandbox(params: { cfg?: AutopusConfig; agentId: string }): string {
...
export function resolveSandboxRuntimeStatus(params: { cfg?: AutopusConfig; sessionKey?: string }): {
  agentId: string;
  sessionKey: string;
...
export function formatSandboxToolPolicyBlockedMessage(params: {
  ...
  const explainCommand = runtime.sessionKey
    ? hasUnsafeControlChars(runtime.sessionKey)
      ? `autopus sandbox explain --agent ${runtime.agentId}`
      : `autopus sandbox explain --session ${shellEscapeSingleArg(runtime.sessionKey)}`
