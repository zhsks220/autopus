...
import { markAutopusExecEnv } from "../../infra/autopus-exec-env.js";
...
export async function ensureDockerImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    throw new Error(
      `Sandbox image not found: ${image}. Build it with scripts/sandbox-setup.sh before enabling Docker sandboxing. The default image includes python3 for sandbox write/edit helpers; Autopus will not substitute plain debian:bookworm-slim.`,
    );
  }
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "autopus.sandbox=1");
  args.push("--label", `autopus.sessionKey=${params.scopeKey}`);
  args.push("--label", `autopus.createdAtMs=${createdAtMs}`);
  args.push("--label", `autopus.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `autopus.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markAutopusExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "autopus.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`autopus sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`autopus sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("autopus sandbox recreate --all");
}
