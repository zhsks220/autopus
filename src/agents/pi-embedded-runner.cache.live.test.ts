...
describeCacheLive("pi embedded runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "autopus-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.AUTOPUS_CACHE_TRACE,
      file: process.env.AUTOPUS_CACHE_TRACE_FILE,
      messages: process.env.AUTOPUS_CACHE_TRACE_MESSAGES,
      prompt: process.env.AUTOPUS_CACHE_TRACE_PROMPT,
      system: process.env.AUTOPUS_CACHE_TRACE_SYSTEM,
    };
    process.env.AUTOPUS_CACHE_TRACE = "1";
    process.env.AUTOPUS_CACHE_TRACE_FILE = liveCacheTraceFile;
    process.env.AUTOPUS_CACHE_TRACE_MESSAGES = "0";
    process.env.AUTOPUS_CACHE_TRACE_PROMPT = "0";
    process.env.AUTOPUS_CACHE_TRACE_SYSTEM = "0";
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("AUTOPUS_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("AUTOPUS_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("AUTOPUS_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("AUTOPUS_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("AUTOPUS_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
