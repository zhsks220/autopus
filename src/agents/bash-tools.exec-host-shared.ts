...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    "Stricter values from tools.exec and ~/.autopus/exec-approvals.json both apply.",
    "Fix one of these:",
    ...
    'Tip: run "autopus doctor" and "autopus approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
