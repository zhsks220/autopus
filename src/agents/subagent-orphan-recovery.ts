 ...
 *
 * @see https://github.com/autopus/autopus/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /autopus\.json|autopus gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumeResult = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify autopus.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
