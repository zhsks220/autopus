...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("autopus-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("autopus-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("autopus-sbx-");
