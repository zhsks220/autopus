...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "autopus-managed-media-"));
...
async function withOutsideHardlinkInAutopusTmp<T>(
  params: {
    autopusTmpDir: string;
  ...
  const hardlinkPath = path.join(params.autopusTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.autopusTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.autopusTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.autopusTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const autopusTmpDir = resolvePreferredAutopusTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Autopus tmp root",
      media: path.join(autopusTmpDir, "image.png"),
      expected: path.join(autopusTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Autopus tmp root",
      media: pathToFileURL(path.join(autopusTmpDir, "photo.png")).href,
      expected: path.join(autopusTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Autopus tmp root",
      media: path.join(autopusTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(autopusTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(autopusTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside autopus tmp root",
      media: path.join(os.tmpdir(), "outside-autopus", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Autopus tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(autopusTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(autopusTmpDir, { recursive: true });
      const symlinkPath = path.join(autopusTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Autopus tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInAutopusTmp(
      {
        autopusTmpDir,
  ...
  it("rejects symlinked Autopus tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInAutopusTmp(
