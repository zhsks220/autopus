...
const ensureAutopusModelsJsonMock = vi.fn(async () => ({ wrote: false }));
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureAutopusModelsJson: (...args: Parameters<typeof ensureAutopusModelsJsonMock>) =>
        ensureAutopusModelsJsonMock(...args),
    };
  });
};
...
beforeAll(async () => {
  ...
  e2eWorkspace = await createEmbeddedPiRunnerTestWorkspace("autopus-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureAutopusModelsJsonMock.mockResolvedValue({ wrote: false });
