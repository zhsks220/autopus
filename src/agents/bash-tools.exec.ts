...
function stripAutopusPackageRunner(argv: string[]): string[] {
  const commandName = normalizeCommandBaseName(argv[0]);
  if (commandName === "autopus") {
    return argv;
  }
  if (
    (commandName === "pnpm" || commandName === "npm" || commandName === "yarn") &&
    normalizeCommandBaseName(argv[1]) === "autopus"
  ) {
    return argv.slice(1);
  }
  if (
    ...
    normalizeCommandBaseName(argv[2]) === "autopus"
  ) {
    return argv.slice(2);
  }
  if (commandName === "npx" || commandName === "bunx") {
    ...
    if (normalizeCommandBaseName(argv[idx]) === "autopus") {
      return argv.slice(idx);
    }
  }
  return argv;
...
function parseAutopusChannelsLoginShellCommand(raw: string): boolean {
  const argv = splitShellArgs(raw);
  if (!argv) {
    return false;
  }
  const autopusArgv = stripAutopusPackageRunner(argv);
  return (
    normalizeCommandBaseName(autopusArgv[0]) === "autopus" &&
    (autopusArgv[1] === "channels" || autopusArgv[1] === "channel") &&
    autopusArgv[2] === "login"
  );
}
...
function rejectUnsafeControlShellCommand(command: string): void {
  ...
  for (const candidate of candidates) {
    ...
    if (parseAutopusChannelsLoginShellCommand(candidate)) {
      throw new Error(
        [
          "exec cannot run interactive Autopus channel login commands.",
          "Run `autopus channels login` in a terminal on the gateway host, or use the channel-specific login agent tool when available (for WhatsApp: `whatsapp_login`).",
        ].join(" "),
      );
    }
  }
