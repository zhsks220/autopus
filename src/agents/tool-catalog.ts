...
function buildCoreToolGroupMap() {
  ...
  const autopusTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInAutopusGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:autopus": autopusTools,
