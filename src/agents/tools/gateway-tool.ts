...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `autopus security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as AutopusConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as AutopusConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isAutopusOwnerOnlyCoreToolName("gateway"),
    description:
