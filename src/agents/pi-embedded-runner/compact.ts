...
import { createAutopusCodingTools, resolveProcessToolScopeKey } from "../pi-tools.js";
...
async function compactEmbeddedPiSessionDirectOnce(
  ...
  try {
    ...
    const toolsRaw = createAutopusCodingTools({
      exec: {
    ...
    const filteredBundledTools = applyFinalEffectiveToolPolicy({
      ...
      // createAutopusCodingTools(...) also omit it, so both paths resolve
    ...
    try {
      ...
      // exact Autopus-managed registrations so custom tools survive startup.
