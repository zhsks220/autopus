...
vi.mock("./models-config.runtime.js", () => ({
  ensureAutopusModelsJson: contextTestState.ensureAutopusModelsJson,
}));
...
function mockContextDeps(params: {
  ...
  contextTestState.ensureAutopusModelsJson.mockClear();
  return { ensureAutopusModelsJson: contextTestState.ensureAutopusModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real autopus startup commands that need model metadata", async () => {
    const { shouldEagerWarmContextWindowCache } = await importContextModule();

    expect(shouldEagerWarmContextWindowCache(["node", "autopus", "chat"])).toBe(true);
    expect(shouldEagerWarmContextWindowCache(["node", "autopus", "chat", "--help"])).toBe(false);
    expect(
      shouldEagerWarmContextWindowCache(["node", "autopus", "matrix", "encryption", "help"]),
    ).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "autopus", "help", "matrix"])).toBe(false);
    expect(
      shouldEagerWarmContextWindowCache(["node", "autopus", "browser", "status", "--help"]),
    ).toBe(false);
    expect(
    ...
    expect(shouldEagerWarmContextWindowCache(["node", "autopus", "logs", "--limit", "5"])).toBe(
      false,
    );
    expect(
      shouldEagerWarmContextWindowCache(["node", "autopus", "memory", "search", "--json"]),
    ).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "autopus", "message", "read"])).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "autopus", "status", "--json"])).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "autopus", "sessions", "--json"])).toBe(
      false,
    );
    expect(
  ...
  it("skips model normalization during warmup but preserves provider-owned context metadata", async () => {
    ...
    expect(
      path.normalize(discoverAgentDir).endsWith(path.join(".autopus", "agents", "main", "agent")),
    ).toBe(true);
