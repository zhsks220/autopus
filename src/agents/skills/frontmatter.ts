...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseAutopusManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyAutopusManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveAutopusMetadata(
  frontmatter: ParsedSkillFrontmatter,
): AutopusSkillMetadata | undefined {
  const metadataObj = resolveAutopusManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveAutopusManifestRequires(metadataObj);
  const install = resolveAutopusManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveAutopusManifestOs(metadataObj);
  return {
