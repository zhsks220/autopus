...
const AUTOPUS_CLAUDE_PLUGIN_NAME = "autopus-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredAutopusTmpDir(), "autopus-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, AUTOPUS_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: AUTOPUS_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Autopus skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
