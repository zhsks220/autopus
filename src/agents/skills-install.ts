...
function resolveDefaultNodeInstallStateDir({
  ...
  if (platform !== "win32" && getuid?.() === 0) {
    return path.join(path.parse(cwd).root, "var", "lib", "autopus");
  }
  return path.join(homedir(), ".autopus");
}
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const trustedInstallSources = new Set(["autopus-bundled", "autopus-managed", "autopus-extra"]);
