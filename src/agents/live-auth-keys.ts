...
const GOOGLE_LIVE_SINGLE_KEY = "AUTOPUS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "AUTOPUS_LIVE_ANTHROPIC_KEY",
    listVar: "AUTOPUS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "AUTOPUS_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `AUTOPUS_LIVE_${base}_KEY`;
