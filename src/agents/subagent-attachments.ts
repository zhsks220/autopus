...
function resolveAttachmentLimits(config: AutopusConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".autopus", "attachments");
  const relDir = path.posix.join(".autopus", "attachments", attachmentId);
