...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  ...
  gatewayPort: 18789,
  displayName: "Mac Studio (Autopus)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "autopus.internal", expected: "autopus.internal." },
    { value: "autopus.internal.", expected: "autopus.internal." },
    { value: "  autopus.internal  ", expected: "autopus.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { AUTOPUS_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { AUTOPUS_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("autopus.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "autopus.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN autopus.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_autopus-gw._tcp IN PTR studio-london._autopus-gw._tcp`,
      `studio-london._autopus-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (Autopus)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/autopus`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN autopus.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._autopus-gw._tcp IN TXT`,
        `displayName=Mac Studio (Autopus)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("autopus.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("autopus.internal."),
      expectedZoneText,
      "utf-8",
    );
  });
