...
import { compareAutopusReleaseVersions } from "./npm-registry-spec.js";
...
export async function fetchNpmPackageTargetStatus(params: {
  ...
  try {
    const res = await fetchWithTimeout(
      `https://registry.npmjs.org/autopus/${encodeURIComponent(target)}`,
      {},
...
export function compareSemverStrings(a: string | null, b: string | null): number | null {
  if (a && b) {
    const autopusReleaseCmp = compareAutopusReleaseVersions(a, b);
    if (autopusReleaseCmp != null) {
      return autopusReleaseCmp;
    }
  }
