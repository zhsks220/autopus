...
function candidateBinDirs(opts: EnsureAutopusPathOpts): { prepend: string[]; append: string[] } {
  ...
  // subprocesses keep using the same Node/Bun the current Autopus process is on.
  try {
  ...
  // Bundled macOS app: `autopus` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "autopus");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.AUTOPUS_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "autopus"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `autopus`
...
/**
 * Best-effort PATH bootstrap so skills that require the `autopus` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAutopusCliOnPath(opts: EnsureAutopusPathOpts = {}) {
  if (isTruthyEnvValue(process.env.AUTOPUS_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.AUTOPUS_PATH_BOOTSTRAPPED = "1";
