...
function createNpmTarget(globalRoot: string): ResolvedGlobalInstallTarget {
  return {
    ...
    command: "npm",
    globalRoot,
    packageRoot: path.join(globalRoot, "autopus"),
  };
}
...
describe("runGlobalPackageUpdateSteps", () => {
  it("installs npm updates into a clean staged prefix before swapping the global package", async () => {
    await withTempDir({ prefix: "autopus-package-update-staged-" }, async (base) => {
      ...
      const runStep = vi.fn(
        async ({ name, argv, cwd, timeoutMs }): Promise<PackageUpdateStepResult> => {
          ...
          await writePackageRoot(path.join(stagePrefix, "lib", "node_modules", "autopus"), "2.0.0");
          await fs.mkdir(path.join(stagePrefix, "bin"), { recursive: true });
          await fs.symlink(
            "../lib/node_modules/autopus/dist/index.js",
            path.join(stagePrefix, "bin", "autopus"),
          );
          return {
            name,
            command: argv.join(" "),
      ...
      await expect(fs.readlink(path.join(prefix, "bin", "autopus"))).resolves.toBe(
        "../lib/node_modules/autopus/dist/index.js",
      );
    });
  });
  ...
  it("accepts v-prefixed exact npm specs when verifying staged installs", async () => {
    await withTempDir({ prefix: "autopus-package-update-v-prefix-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "autopus");
      await writePackageRoot(packageRoot, "1.0.0");
      ...
      const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
        ...
        expect(argv).toContain("autopus@v2.0.0");
        const prefixIndex = argv.indexOf("--prefix");
        const stagePrefix = argv[prefixIndex + 1];
        if (!stagePrefix) {
          throw new Error("missing staged prefix");
  ...
  it("swaps staged npm package roots through the copy fallback when rename crosses devices", async () => {
    await withTempDir({ prefix: "autopus-package-update-exdev-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "autopus");
      ...
      const renameSpy = vi
        ...
        .mockImplementation(async (...args: Parameters<typeof fs.rename>) => {
          ...
          if (
            exdevMoves === 0 &&
            fromPath.includes(`${path.sep}.autopus-update-stage-`) &&
            path.basename(fromPath) === "autopus" &&
        ...
        const result = await runGlobalPackageUpdateSteps({
          ...
          runStep: async ({ name, argv, cwd }) => {
            ...
            await writePackageRoot(
              path.join(stagePrefix, "lib", "node_modules", "autopus"),
              "2.0.0",
            );
            return {
              name,
  ...
  it("stages pnpm-detected updates through npm when the global root has npm prefix layout", async () => {
    await withTempDir({ prefix: "autopus-package-update-pnpm-staged-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "autopus");
      ...
      const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
        ...
        expect(argv).toContain("autopus@2.0.0");
  ...
  it("keeps Windows pnpm global roots on the pnpm update path", async () => {
    ...
    try {
      await withTempDir({ prefix: "autopus-package-update-win32-pnpm-" }, async (base) => {
        ...
        const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
          ...
          expect(argv).toEqual(["pnpm", "add", "-g", "--global-dir", globalDir, "autopus@2.0.0"]);
  ...
  it("keeps a successful staged swap when old package cleanup hits a transient Windows native module error", async () => {
    await withTempDir({ prefix: "autopus-package-update-staged-cleanup-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "autopus");
      await writePackageRoot(packageRoot, "1.0.0");
      ...
      const rmSpy = vi.spyOn(fs, "rm").mockImplementation(async (target, options) => {
        ...
        if (
          targetPath.includes(`${path.sep}.autopus-`) &&
          !targetPath.includes(".autopus-update-stage-") &&
          !targetPath.includes(".autopus-shim-backup-")
        ) {
  ...
  it("does not run post-verify work when staged npm verification fails", async () => {
    await withTempDir({ prefix: "autopus-package-update-verify-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "autopus");
      await writePackageRoot(packageRoot, "1.0.0");
      ...
      const result = await runGlobalPackageUpdateSteps({
        ...
        runStep: async ({ name, argv, cwd }) => {
          ...
          await writePackageRoot(path.join(stagePrefix, "lib", "node_modules", "autopus"), "1.5.0");
          return {
            name,
            command: argv.join(" "),
            cwd: cwd ?? process.cwd(),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "autopus-package-update-shim-rollback-" }, async (base) => {
        const prefix = path.join(base, "prefix");
        const globalRoot = path.join(prefix, "lib", "node_modules");
        const packageRoot = path.join(globalRoot, "autopus");
        const targetShim = path.join(prefix, "bin", "autopus");
        await writePackageRoot(packageRoot, "1.0.0");
        ...
        try {
          result = await runGlobalPackageUpdateSteps({
            ...
            runStep: async ({ name, argv, cwd }) => {
              ...
              const stagedShim = path.join(stagePrefix, "bin", "autopus");
  ...
  it("cleans the staged npm prefix when the install command throws", async () => {
    await withTempDir({ prefix: "autopus-package-update-cleanup-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "autopus");
      await writePackageRoot(packageRoot, "1.0.0");
