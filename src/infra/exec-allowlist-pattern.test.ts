...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevAutopusHome = process.env.AUTOPUS_HOME;
    const prevHome = process.env.HOME;
    process.env.AUTOPUS_HOME = "/srv/autopus-home";
    process.env.HOME = "/home/other";
    const autopusHome = path.join(path.resolve("/srv/autopus-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", autopusHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevAutopusHome === undefined) {
        delete process.env.AUTOPUS_HOME;
      } else {
        process.env.AUTOPUS_HOME = prevAutopusHome;
      }
