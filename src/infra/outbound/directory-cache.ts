...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AutopusConfig | null = null;
  ...
  get(key: string, cfg: AutopusConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AutopusConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: AutopusConfig): void {
