...
describe("resolveLocalVitestMaxWorkers", () => {
  ...
  it("lets AUTOPUS_VITEST_MAX_WORKERS override the inferred cap", () => {
    expect(
      resolveLocalVitestMaxWorkers(
        {
  ...
  it("respects the legacy AUTOPUS_TEST_WORKERS override too", () => {
    expect(
      resolveLocalVitestMaxWorkers(
        {
          AUTOPUS_TEST_WORKERS: "3",
        },
        {
          cpuCount: 16,
          loadAverage1m: 0,
...
describe("resolveLocalVitestScheduling", () => {
  ...
  it("allows disabling the system throttle probe explicitly", () => {
    expect(
      resolveLocalVitestScheduling(
        {
          AUTOPUS_VITEST_DISABLE_SYSTEM_THROTTLE: "1",
        },
        {
          cpuCount: 16,
          loadAverage1m: 0.5,
...
describe("test scripts", () => {
  it("keeps test scripts on the native thread-first configs", () => {
    ...
    expect(pkg.scripts?.["test:serial"]).toBe(
      "AUTOPUS_TEST_PROJECTS_SERIAL=1 AUTOPUS_VITEST_MAX_WORKERS=1 node scripts/test-projects.mjs",
    );
    ...
    expect(pkg.scripts?.["test:gateway"]).toBe(
      "AUTOPUS_GATEWAY_PROJECT_SHARDS=1 node scripts/run-vitest.mjs run --config test/vitest/vitest.gateway.config.ts",
    );
