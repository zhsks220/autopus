...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@autopus/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@autopus/matrix", id: "matrix", expected: true },
    { packageName: "@autopus/matrix", id: "@autopus/matrix", expected: true },
    { packageName: "@autopus/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@autopus/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@autopus__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "autopus-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "autopus-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "autopus-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "autopus-install-safe-" }, async (parentDir) => {
