...
vi.mock("./autopus-root.js", async () => {
  const actual = await vi.importActual<typeof import("./autopus-root.js")>("./autopus-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "autopus-update-check-suite-" });
  let tempDir: string;
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["AUTOPUS_NO_AUTO_UPDATE", "AUTOPUS_STATE_DIR", "NODE_ENV", "VITEST"]);
    ...
    if (!loaded) {
      ({ resolveAutopusPackageRoot } = await import("./autopus-root.js"));
    ...
    vi.mocked(resolveAutopusPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveAutopusPackageRoot).mockResolvedValue("/opt/autopus");
  ...
  it.each([
    ...
    expect(log.info).toHaveBeenCalledWith(
      `update available (latest): v2.0.0 (current v1.0.0). Run: ${formatCliCommand("autopus update")}`,
    );
  ...
  it("honors AUTOPUS_NO_AUTO_UPDATE for configured auto-updates", async () => {
    mockPackageUpdateStatus("beta", "2.0.0-beta.1");
    process.env.AUTOPUS_NO_AUTO_UPDATE = "1";
    const log = { info: vi.fn() };
    const runAutoUpdate = createAutoUpdateSuccessMock();
    ...
    const disabledLogCall = log.info.mock.calls.find(
      ([message]) => message === "auto-update disabled by AUTOPUS_NO_AUTO_UPDATE",
    );
    expect(disabledLogCall).toEqual([
      "auto-update disabled by AUTOPUS_NO_AUTO_UPDATE",
      {
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/autopus/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
