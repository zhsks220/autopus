...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open Autopus to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "autopus.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.AUTOPUS_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.AUTOPUS_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set AUTOPUS_APNS_TEAM_ID and AUTOPUS_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.AUTOPUS_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.AUTOPUS_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.AUTOPUS_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set AUTOPUS_APNS_PRIVATE_KEY_P8 or AUTOPUS_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading AUTOPUS_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
