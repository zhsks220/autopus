...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.AUTOPUS_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Autopus. You can keep running the web gateway without it: \`${formatCliCommand("autopus gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
