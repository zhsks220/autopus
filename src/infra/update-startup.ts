...
import { resolveAutopusPackageRoot } from "./autopus-root.js";
...
function resolveAutoUpdatePolicy(cfg: AutopusConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: AutopusConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.AUTOPUS_NO_AUTO_UPDATE);
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("autopus update")}`,
      );
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by AUTOPUS_NO_AUTO_UPDATE", {
