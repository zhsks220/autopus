...
async function makeTempDir(label: string): Promise<string> {
  return await tempDirs.make(`autopus-${label}-`);
}
...
describe("git commit resolution", () => {
  ...
  it("does not walk out of the autopus package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "autopus");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "autopus", version: "2026.3.10" }),
      "utf-8",
    );
