...
describe("logAcceptedEnvOption", () => {
  it("logs accepted env options once with redaction and formatting", async () => {
    ...
    withEnv(
      {
        ...
        AUTOPUS_TEST_ENV: "  line one\nline two  ",
      },
      () => {
        logAcceptedEnvOption({
          key: "AUTOPUS_TEST_ENV",
    ...
    expect(loggerMocks.info).toHaveBeenCalledWith("env: AUTOPUS_TEST_ENV=<redacted> (test option)");
  });
    ...
    withEnv(
      {
        VITEST: "1",
        NODE_ENV: "development",
        AUTOPUS_BLANK_ENV: "value",
      },
      () => {
        logAcceptedEnvOption({
          key: "AUTOPUS_BLANK_ENV",
