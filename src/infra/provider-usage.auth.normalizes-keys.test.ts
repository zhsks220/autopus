...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "autopus-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".autopus");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".autopus", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      AUTOPUS_STATE_DIR: path.join(home, ".autopus"),
      ...env,
    };
  ...
  async function writeConfig(home: string, config: Record<string, unknown>) {
    ...
    await fs.writeFile(
      path.join(stateDir, "autopus.json"),
