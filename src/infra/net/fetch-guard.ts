...
const AUTOPUS_DEBUG_PROXY_ENABLED = "AUTOPUS_DEBUG_PROXY_ENABLED";
...
function isManagedProxyActive(): boolean {
  return process.env["AUTOPUS_PROXY_ACTIVE"] === "1";
}
...
async function captureGuardedFetchExchange(params: {
  ...
  if (params.capture === false || !isTruthyEnvValue(process.env[AUTOPUS_DEBUG_PROXY_ENABLED])) {
    return;
  }
