...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.AUTOPUS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*autopus-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "autopus";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "autopus");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "autopus-gw");
  ...
  records.push(`_autopus-gw._tcp IN PTR ${instanceLabel}._autopus-gw._tcp`);
  records.push(`${instanceLabel}._autopus-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._autopus-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; autopus-content-hash: ${contentHash}\n${contentBody}`;
}
