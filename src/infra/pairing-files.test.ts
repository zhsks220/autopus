...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/autopus-state", "devices")).toEqual({
      dir: path.join("/tmp/autopus-state", "devices"),
      pendingPath: path.join("/tmp/autopus-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/autopus-state", "devices", "paired.json"),
    });
  });
