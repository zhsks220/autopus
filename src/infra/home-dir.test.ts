...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers AUTOPUS_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for AUTOPUS_HOME",
      env: { AUTOPUS_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when AUTOPUS_HOME is tilde-only and no fallback home exists",
      env: { AUTOPUS_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores AUTOPUS_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          AUTOPUS_HOME: "/srv/autopus-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/autopus-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/autopus-home " },
      expected: "/srv/autopus-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/autopus-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of AUTOPUS_HOME", () => {
    expect(
