...
let compiledSecretTargetRegistryState: {
  ...
  autopusCompiledSecretTargets: CompiledTargetRegistryEntry[];
  autopusTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
} | null = null;

let compiledCoreAutopusTargetState: {
  knownTargetIds: Set<string>;
  autopusCompiledSecretTargets: CompiledTargetRegistryEntry[];
  autopusTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
...
const compiledChannelAutopusTargets = new Map<string, CompiledTargetRegistryEntry[] | null>();
...
function getCompiledSecretTargetRegistryState() {
  ...
  const autopusCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "autopus.json",
  );
  ...
  compiledSecretTargetRegistryState = {
    ...
    autopusCompiledSecretTargets,
    autopusTargetsById: buildConfigTargetIdIndex(autopusCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(compiledSecretTargetRegistry),
  };
  return compiledSecretTargetRegistryState;
}

function getCompiledCoreAutopusTargetState() {
  if (compiledCoreAutopusTargetState) {
    return compiledCoreAutopusTargetState;
  }
  const autopusCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "autopus.json")
    .map(compileTargetRegistryEntry);
  compiledCoreAutopusTargetState = {
    knownTargetIds: new Set(autopusCompiledSecretTargets.map((entry) => entry.id)),
    autopusCompiledSecretTargets,
    autopusTargetsById: buildConfigTargetIdIndex(autopusCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(autopusCompiledSecretTargets),
  };
  return compiledCoreAutopusTargetState;
}

function getCompiledChannelAutopusTargets(channelId: string): CompiledTargetRegistryEntry[] | null {
  const normalizedChannelId = channelId.trim();
  if (!normalizedChannelId) {
    return null;
  }
  if (compiledChannelAutopusTargets.has(normalizedChannelId)) {
    return compiledChannelAutopusTargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadChannelSecretContractApi({
      ...
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "autopus.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledChannelAutopusTargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreAutopusTargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  for (const entry of getCompiledCoreAutopusTargetState().autopusCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
  ...
  const explicitChannelEntries = explicitChannelId
    ? getCompiledChannelAutopusTargets(explicitChannelId)
    : null;
  ...
  for (const entry of getCompiledSecretTargetRegistryState().autopusCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
    const matched = matchPathTokens(pathSegments, entry.pathTokens);
...
export function discoverConfigSecretTargets(config: AutopusConfig): DiscoveredConfigSecretTarget[] {
...
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreAutopusTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreAutopusTargetState()
      : getCompiledSecretTargetRegistryState();
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: registryState.autopusCompiledSecretTargets,
    entriesById: registryState.autopusTargetsById,
  });
