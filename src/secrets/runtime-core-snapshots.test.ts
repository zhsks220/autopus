...
describe("secrets runtime snapshot core lanes", () => {
  ...
  it("resolves env-backed auth profile SecretRefs", async () => {
    ...
    expect(warningPaths).toContain("/tmp/autopus-agent-main.auth-profiles.openai:default.key");
    expect(warningPaths).toContain(
      "/tmp/autopus-agent-main.auth-profiles.github-copilot:default.token",
    );
  ...
  it("activates runtime snapshots for ensureAuthProfileStore", async () => {
    ...
    const runtimeProfile = ensureAuthProfileStore("/tmp/autopus-agent-main").profiles[
      "openai:default"
