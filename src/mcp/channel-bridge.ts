...
export class AutopusChannelBridge {
  ...
  constructor(
    private readonly cfg: AutopusConfig,
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`autopus mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `autopus mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
