...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "/tmp/fake-autopus.json",
    ...
    expect(
      shouldSkipLegacyUpdateDoctorConfigWrite({
        env: {
          ...
          AUTOPUS_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE: "1",
        },
      }),
    ).toBe(false);
  });
  ...
  describe("config size drops during update", () => {
    ...
    function buildWriteConfigCtx(env: Record<string, string | undefined>) {
      ...
      return {
        ...
        configPath: "/tmp/fake-autopus.json",
        sourceConfigValid: true,
        prompter: buildDoctorPrompter(true),
    ...
    it("allows config size drops when AUTOPUS_UPDATE_IN_PROGRESS=1", async () => {
    ...
    it("points update-time config rewrites at the pre-update backup", async () => {
      ...
      expect(ctx.runtime.log).toHaveBeenCalledWith(
        "Update changed config; pre-update backup: /tmp/fake-autopus.json.pre-update",
      );
    });
