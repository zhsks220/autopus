...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[autopus proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[autopus proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`autopus proxy coverage\` for details.`,
  );
}
