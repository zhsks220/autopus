...
const TEMP_MEDIA_PREFIX = "autopus-media-";
...
function createGroqAudioConfig(): AutopusConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): AutopusConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): AutopusConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: AutopusConfig = { tools: { media: { audio: {} } } };
