...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("autopus status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("autopus security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set AUTOPUS_GATEWAY_TOKEN/AUTOPUS_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("autopus security audit --deep")}".`,
    });
  }
  return findings;
}
