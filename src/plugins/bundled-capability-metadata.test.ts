  ...
  return fs
    ...
    .filter((pluginDir) => {
      ...
      const packageJson = JSON.parse(fs.readFileSync(packagePath, "utf-8")) as {
        autopus?: AutopusPackageManifest;
      };
      return normalizeBundledPluginStringList(packageJson.autopus?.extensions).length > 0;
    })
    .map(
      (pluginDir) =>
        JSON.parse(
          fs.readFileSync(path.join(pluginDir, "autopus.plugin.json"), "utf-8"),
        ) as PluginManifest,
    )
