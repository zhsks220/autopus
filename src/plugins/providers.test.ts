...
type LoadAutopusPlugins = typeof import("./loader.js").loadAutopusPlugins;
...
const loadAutopusPluginsMock = vi.fn<LoadAutopusPlugins>();
...
function expectLastSetupRegistryCall(params: {
  ...
  const call = getLastMockCallArg(loadAutopusPluginsMock, "Autopus plugin setup loader");
...
function getLastSetupLoadedPluginConfig() {
  const call = expectRecordFields(
    getLastMockCallArg(loadAutopusPluginsMock, "Autopus plugin setup loader"),
    {},
  );
...
function createAutoEnabledProviderConfig() {
  const rawConfig: AutopusConfig = {
    plugins: {},
  };
  const autoEnabledConfig: AutopusConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadAutopusPlugins: (...args: Parameters<LoadAutopusPlugins>) =>
        loadAutopusPluginsMock(...args),
