...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("autopus-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("autopus-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("autopus-marketplace-test-", async (homeDir) => {
      const autopusHome = path.join(homeDir, "autopus-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, AUTOPUS_HOME: autopusHome },
