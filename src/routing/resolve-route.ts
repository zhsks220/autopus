...
function listAgents(cfg: AutopusConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: AutopusConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<AutopusConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: AutopusConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: AutopusConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: AutopusConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<AutopusConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  AutopusConfig,
  {
    bindingsRef: AutopusConfig["bindings"];
    agentsRef: AutopusConfig["agents"];
    sessionRef: AutopusConfig["session"];
...
function resolveRouteCacheForConfig(cfg: AutopusConfig): Map<string, ResolvedAgentRoute> {
