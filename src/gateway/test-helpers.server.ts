...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.AUTOPUS_CONFIG_PATH) {
    configPaths.add(process.env.AUTOPUS_CONFIG_PATH);
  }
  if (process.env.AUTOPUS_STATE_DIR) {
    configPaths.add(path.join(process.env.AUTOPUS_STATE_DIR, "autopus.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "autopus-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.AUTOPUS_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "autopus-test-no-bundled-extensions")
    : "autopus-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".autopus-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".autopus-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".autopus-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>autopus-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__autopusTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__autopusTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["AUTOPUS_GATEWAY_TOKEN"]);
  const prev = process.env.AUTOPUS_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.AUTOPUS_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.AUTOPUS_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
