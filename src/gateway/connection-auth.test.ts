...
function cfg(input: Partial<AutopusConfig>): AutopusConfig {
  return input as AutopusConfig;
}
...
const DEFAULT_ENV = {
  ...
  AUTOPUS_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when AUTOPUS env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when AUTOPUS env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when AUTOPUS env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
