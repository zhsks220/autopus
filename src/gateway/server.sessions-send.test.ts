...
const { createAutopusTools } = await import("../agents/autopus-tools.js");
...
type SessionSendTool = ReturnType<typeof createAutopusTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createAutopusTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["AUTOPUS_GATEWAY_PORT", "AUTOPUS_GATEWAY_TOKEN"]);
  ...
  process.env.AUTOPUS_GATEWAY_PORT = String(gatewayPort);
  process.env.AUTOPUS_GATEWAY_TOKEN = gatewayToken;
