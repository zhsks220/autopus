...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "autopus-test-owner-display-secret",
    },
  };
}
...
export async function withTempConfig(params: {
  ...
  const prevConfigPath = process.env.AUTOPUS_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as AutopusConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "autopus-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.AUTOPUS_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
