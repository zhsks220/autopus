...
type GatewayInstallAuthMode = NonNullable<NonNullable<AutopusConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: AutopusConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    normalizeOptionalString(durableServiceEnv.AUTOPUS_GATEWAY_PASSWORD) ||
