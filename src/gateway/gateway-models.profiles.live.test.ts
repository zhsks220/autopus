...
import type { ModelsConfig, ModelProviderConfig, AutopusConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.AUTOPUS_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.AUTOPUS_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.AUTOPUS_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.AUTOPUS_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_SETUP_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.AUTOPUS_LIVE_GATEWAY_SETUP_TIMEOUT_MS, 60_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.AUTOPUS_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.AUTOPUS_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["AUTOPUS_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.AUTOPUS_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.AUTOPUS_LIVE_GATEWAY_MODELS?.trim();
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.AUTOPUS_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.AUTOPUS_LIVE_MODEL_TIMEOUT_MS,
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.AUTOPUS_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.AUTOPUS_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.AUTOPUS_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("AUTOPUS_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("AUTOPUS_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("AUTOPUS_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.AUTOPUS_LIVE_GATEWAY_MODELS;
    delete process.env.AUTOPUS_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.AUTOPUS_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.AUTOPUS_LIVE_GATEWAY_MODELS = "openai/gpt-5.5,anthropic/claude-opus-4-6";
    delete process.env.AUTOPUS_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.AUTOPUS_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.AUTOPUS_LIVE_GATEWAY_MAX_MODELS = "2";
...
async function sanitizeAuthConfig(params: {
  cfg: AutopusConfig;
  agentDir: string;
}): Promise<AutopusConfig["auth"] | undefined> {
  ...
  let profiles: NonNullable<AutopusConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    ...
    disableBonjour: process.env.AUTOPUS_DISABLE_BONJOUR,
    logLevel: process.env.AUTOPUS_LOG_LEVEL,
  ...
  if (QUIET_LIVE_LOGS) {
    process.env.AUTOPUS_DISABLE_BONJOUR = "1";
    process.env.AUTOPUS_LOG_LEVEL = "silent";
  }
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "autopus-live-state-"));
  ...
  await fs.mkdir(path.join(workspaceDir, ".autopus"), { recursive: true });
  await fs.writeFile(
    path.join(workspaceDir, ".autopus", "workspace-state.json"),
    `${JSON.stringify(
      {
        version: 1,
  ...
  const toolProbePath = path.join(workspaceDir, `.autopus-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveDefaultAgentDir(params.cfg);
  const sanitizedCfg: AutopusConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "autopus-live-"));
  ...
  try {
            ...
            (async () => {
              ...
              for (
                ...
                toolText = await requestGatewayAgentText({
                  ...
                  modelKey,
                  message: strictReply
                    ? "Autopus live tool probe (local, safe): " +
    ...
    process.env.AUTOPUS_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.AUTOPUS_LOG_LEVEL = previous.logLevel;
    process.env.AUTOPUS_AGENT_DIR = previous.agentDir;
...
describeLive("gateway live (dev agent, profile keys)", () => {
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        await withGatewayLiveSetupTimeout(
          ensureAutopusModelsJson(cfg, undefined, {
            workspaceDir,
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via AUTOPUS_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.autopus-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
