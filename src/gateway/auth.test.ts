...
describe("gateway auth", () => {
  ...
  it("resolves token/password from AUTOPUS gateway env vars", () => {
    const auth = resolveGatewayAuth({
      authConfig: {},
      env: {
        AUTOPUS_GATEWAY_TOKEN: "env-token",
        AUTOPUS_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides AUTOPUS_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        AUTOPUS_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
