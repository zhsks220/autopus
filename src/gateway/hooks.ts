...
export function resolveHooksConfig(cfg: AutopusConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: AutopusConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-autopus-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-autopus-idempotency-key"]) ||
