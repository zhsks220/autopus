...
type HomeEnvSnapshot = {
  ...
  AUTOPUS_STATE_DIR: string | undefined;
  AUTOPUS_AGENT_DIR: string | undefined;
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".autopus", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.AUTOPUS_STATE_DIR = path.join(home, ".autopus");
    process.env.AUTOPUS_AGENT_DIR = path.join(home, ".autopus", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".autopus", "agent");
