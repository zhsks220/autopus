...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "autopus-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop autopus!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@autopus_bot", { botUsername: "autopus_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@autopus_bot", { botUsername: "autopus_bot" })).toBe(true);
