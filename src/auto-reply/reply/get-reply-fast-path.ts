...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("autopus.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("autopus.reply.full-runtime");

type ReplyConfigWithMarker = AutopusConfig & {
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.AUTOPUS_ALLOW_SLOW_REPLY_TESTS === "1" || env.AUTOPUS_STRICT_FAST_REPLY_CONFIG === "0";
}
...
export function markCompleteReplyConfig<T extends AutopusConfig>(
  config: T,
...
export function withFastReplyConfig<T extends AutopusConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends AutopusConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

function isCompleteReplyConfig(config: unknown): config is AutopusConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set AUTOPUS_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  ...
  return applyMergePatch(params.getRuntimeConfig(), configOverride) as AutopusConfig;
}
