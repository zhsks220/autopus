...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".autopus");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.AUTOPUS_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$AUTOPUS_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via AUTOPUS_STATE_DIR.
