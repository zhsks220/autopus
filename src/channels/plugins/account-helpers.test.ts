...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): AutopusConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as AutopusConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: AutopusConfig) => string[];
  input: AutopusConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: AutopusConfig, expected: string) {
