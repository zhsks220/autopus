...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.AUTOPUS_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveAutopusPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    AUTOPUS_STATE_DIR: path.join(os.tmpdir(), "autopus-config-doc-baseline-state"),
