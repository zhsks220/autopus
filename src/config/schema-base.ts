...
function computeBaseConfigSchemaStablePayload(): BaseConfigSchemaStablePayload {
  ...
  const schema = AutopusSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "AutopusConfig";
  const schemaRoot = asJsonSchemaObject(schema);
  if (schemaRoot) {
    applyFieldDocumentation(schemaRoot, buildFieldDocumentation());
  }
  const baseHints = mapSensitivePaths(AutopusSchema, "", buildBaseHints());
  const sensitiveUrlPaths = collectMatchingSchemaPaths(AutopusSchema, "", isSensitiveUrlConfigPath);
  const stablePayload = {
