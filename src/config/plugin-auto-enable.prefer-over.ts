...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.autopus) || !isRecord(entry.autopus.channel)) {
      continue;
    }
    const channel = entry.autopus.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: AutopusConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: AutopusConfig, pluginId: string) => boolean;
