...
declare const __AUTOPUS_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "autopus";
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["AUTOPUS_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["AUTOPUS_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["AUTOPUS_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.AUTOPUS_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current Autopus version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __AUTOPUS_VERSION__ === "string" ? __AUTOPUS_VERSION__ : undefined,
  bundledVersion: process.env.AUTOPUS_BUNDLED_VERSION,
});
