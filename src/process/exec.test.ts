...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        AUTOPUS_BASE_ENV: "base",
        AUTOPUS_TO_REMOVE: undefined,
      },
      env: {
        AUTOPUS_TEST_ENV: "ok",
      },
    ...
    expect(resolved.AUTOPUS_BASE_ENV).toBe("base");
    expect(resolved.AUTOPUS_TEST_ENV).toBe("ok");
    expect(resolved.AUTOPUS_TO_REMOVE).toBeUndefined();
    expect(resolved.AUTOPUS_CLI).toBe(AUTOPUS_CLI_ENV_VALUE);
  });
