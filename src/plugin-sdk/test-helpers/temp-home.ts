...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked AUTOPUS_HOME.
  delete process.env.AUTOPUS_HOME;
  process.env.AUTOPUS_STATE_DIR = path.join(base, ".autopus");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "autopus-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".autopus", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
