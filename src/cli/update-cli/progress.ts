...
const STEP_LABELS: Record<string, string> = {
  ...
  "autopus doctor entry": "Checking doctor entrypoint",
  "autopus doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g autopus@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g autopus@latest --omit=optional");
  }
