...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.autopus.ai/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option("-i, --install", t("opt.install_completion_script_to_shell_profile"))
    .option(
      "--write-state",
      "Write completion scripts to $AUTOPUS_STATE_DIR/completions (no stdout)",
    )
    .option("-y, --yes", t("opt.skip_confirmation_non_interactive"), false)
