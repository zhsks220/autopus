...
import { readBestEffortConfig, type AutopusConfig } from "../config/config.js";
...
function registerAllowlistMutationCommand(params: {
  ...
  const command = params.allowlist
    ...
    .option("--node <node>", t("opt.target_node_id_name_ip"))
    .option("--gateway", t("opt.force_gateway_approvals"), false)
...
export function registerExecApprovalsCli(program: Command) {
  ...
  const approvals = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/approvals", "docs.autopus.ai/cli/approvals")}\n`,
    );

  const getCmd = approvals
    .command("get")
    .description(t("desc.fetch_exec_approvals_snapshot"))
    .option("--node <node>", t("opt.target_node_id_name_ip"))
    .option("--gateway", t("opt.force_gateway_approvals"), false)
  ...
  const setCmd = approvals
    .command("set")
    .description(t("desc.replace_exec_approvals_with_a_json_file"))
    .option("--node <node>", t("opt.target_node_id_name_ip"))
    .option("--gateway", t("opt.force_gateway_approvals"), false)
    .option("--file <path>", t("opt.path_to_json_file_to_upload"))
    .option("--stdin", t("opt.read_json_from_stdin"), false)
    .action(async (opts: ExecApprovalsCliOpts) => {
      try {
  ...
  const allowlist = approvals
    .command("allowlist")
    .description(t("desc.edit_the_per_agent_allowlist"))
    .addHelpText(
      "after",
      () =>
