...
export const COMPLETION_SKIP_PLUGIN_COMMANDS_ENV = "AUTOPUS_COMPLETION_SKIP_PLUGIN_COMMANDS";
...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Autopus Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# Autopus Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(autopus completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "autopus") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
