...
function createLogWriters() {
  const writer = createSafeStreamWriter({
    ...
    onBrokenPipe: (err, stream) => {
      ...
      const message = `autopus logs: output ${target} closed (${code}). Stopping tail.`;
      try {
...
async function emitGatewayError(
  ...
  const hint = `Hint: run \`${formatCliCommand("autopus doctor")}\`.`;
...
export function registerLogsCli(program: Command) {
  const logs = program
    .command("logs")
    .description(t("desc.tail_gateway_file_logs_via_rpc"))
    .option("--limit <n>", t("opt.max_lines_to_return"), "200")
    .option("--max-bytes <n>", t("opt.max_bytes_to_read"), "250000")
    .option("--follow", t("opt.follow_log_output"), false)
    .option("--interval <ms>", t("opt.polling_interval_in_ms"), "1000")
    .option("--json", t("opt.emit_json_log_lines"), false)
    .option("--plain", t("opt.plain_text_output_no_ansi_styling"), false)
    .option("--no-color", t("opt.disable_ansi_colors"))
    .option("--local-time", t("opt.display_timestamps_in_local_timezone"), false)
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/logs", "docs.autopus.ai/cli/logs")}\n`,
    );
