...
export function registerExecPolicyCli(program: Command) {
  ...
  execPolicy
    .command("show")
    .description(t("desc.show_the_local_config_policy_host_approvals_and_effective_merge"))
  ...
  execPolicy
    .command("set")
    .description(t("desc.synchronize_local_config_and_host_approvals_using_explicit_values"))
    .option("--host <host>", t("opt.exec_host_target_auto_sandbox_gateway_node"))
    .option("--security <mode>", t("opt.exec_security_deny_allowlist_full"))
    .option("--ask <mode>", t("opt.exec_ask_mode_off_on_miss_always"))
    .option("--ask-fallback <mode>", t("opt.host_approvals_fallback_deny_allowlist_full"))
    .option("--json", t("opt.output_as_json"), false)
    .action(
      async (opts: {
        host?: string;
