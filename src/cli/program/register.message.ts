...
export function registerMessageCommands(program: Command, ctx: ProgramContext) {
  const message = program
    .command("message")
    .description(t("desc.send_read_and_manage_messages_and_channel_actions"))
    .addHelpText(
      "after",
      () =>
        `
...
${formatHelpExamples([
  ['autopus message send --target +15555550123 --message "Hi"', "Send a text message."],
  [
    'autopus message send --target +15555550123 --message "Hi" --media photo.jpg',
    "Send a message with media.",
  ],
  [
    'autopus message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi',
    "Create a Discord poll.",
  ],
  [
    'autopus message react --channel discord --target 123 --message-id 456 --emoji "✅"',
    "React to a message.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/message", "docs.autopus.ai/cli/message")}`,
    )
    .action(() => {
