...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "autopus", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "autopus", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "autopus", "--profile", "work", "gateway", "status"],
