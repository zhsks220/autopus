...
/**
 * Test for issue #6070:
 * `autopus config set/unset` must update snapshot.resolved (user config after $include/${ENV},
...
function setSnapshot(resolved: AutopusConfig, config: AutopusConfig) {
...
function withRuntimeDefaults(resolved: AutopusConfig): AutopusConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
function makeInvalidSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/custom-autopus.json",
    exists: true,
    raw: "{}",
    parsed: {},
    sourceConfig: {},
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: AutopusConfig = {
    ...
    it("rejects plugin install record config updates", async () => {
      await expect(
        runConfigCommand([
          "config",
          "set",
          'plugins.installs["autopus-web-search"].spec',
          '"@ollama/autopus-web-search@0.2.2"',
          "--strict-json",
          "--dry-run",
        ]),
      ...
      expectErrorIncludes("autopus plugins install <spec>");
      expectErrorIncludes("autopus plugins update <plugin-id>");
    });
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__AUTOPUS_REDACTED__");
    });
  });
  ...
  describe("config validate", () => {
    ...
    it("returns machine-readable JSON with --json for invalid config", async () => {
      ...
      expect(payload.path).toBe("/tmp/custom-autopus.json");
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: AutopusConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
    ...
    it("shows --strict-json and keeps --json as a legacy alias in help", () => {
      ...
      expect(helpText).toContain("autopus config set gateway.port 19001 --strict-json");
      expect(helpText).toContain(
        "autopus config set channels.discord.token --ref-provider default --ref-source",
      );
  ...
  describe("config set builders and dry-run", () => {
    ...
    it("dry-runs config patch channel fields against plugin-owned schemas", async () => {
      ...
      setSnapshot(resolved, resolved);
      const pathname = writeTempJson5File("autopus-config-plugin-channel-schema", {
        channels: {
          feishu: {
            appId: "app-id",
            appSecret: "secret",
    ...
    it("rejects --allow-exec without --dry-run", async () => {
      const nonexistentBatchPath = path.join(
        os.tmpdir(),
        `autopus-config-batch-nonexistent-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      await expect(
    ...
    it("supports batch-file mode", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `autopus-config-batch-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
    ...
    it("batch-file nested leaf updates preserve agents defaults and list siblings", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `autopus-config-memory-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify([
    ...
    it("rejects malformed batch-file payloads", async () => {
      const pathname = path.join(
        os.tmpdir(),
        `autopus-config-batch-invalid-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      fs.writeFileSync(pathname, '{"path":"gateway.auth.mode","value":"token"}', "utf8");
      try {
    ...
    it("patches config from one object in one write", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `autopus-config-patch-${Date.now()}-${Math.random().toString(16).slice(2)}.json5`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify({
    ...
    it("preserves empty object values in config patch", async () => {
      ...
      const pathname = writeTempJson5File("autopus-config-patch-empty-object", {
        agents: {
          defaults: {
            models: {
              "openai/gpt-5.5": {},
    ...
    it("treats empty object config patches as recursive merges", async () => {
      ...
      const pathname = writeTempJson5File("autopus-config-patch-empty-merge", {
        channels: {
          slack: {},
        },
      });
    ...
    it("dry-runs config patch and resolves changed SecretRefs", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `autopus-config-patch-dry-${Date.now()}-${Math.random().toString(16).slice(2)}.json5`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify({
    ...
    it("schema-validates SecretRef-only config patch operations", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `autopus-config-patch-ref-schema-${Date.now()}-${Math.random()
          .toString(16)
          .slice(2)}.json5`,
      );
      fs.writeFileSync(
    ...
    it("dry-runs nested SecretRefs inside config patch replacements", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `autopus-config-patch-nested-ref-${Date.now()}-${Math.random()
          .toString(16)
          .slice(2)}.json5`,
      );
      fs.writeFileSync(
    ...
    it("supports replace-path and null deletes in config patch", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `autopus-config-patch-replace-${Date.now()}-${Math.random().toString(16).slice(2)}.json5`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify({
    ...
    it("rejects unused config patch replace paths", async () => {
      const pathname = path.join(
        os.tmpdir(),
        `autopus-config-patch-unused-replace-${Date.now()}-${Math.random()
          .toString(16)
          .slice(2)}.json5`,
      );
      fs.writeFileSync(
  ...
  describe("config file", () => {
    it("prints the active config file path", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/tmp/autopus.json");
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
    });
    ...
    it("handles config file path with home directory", async () => {
      ...
      snapshot.path = "/home/user/.autopus/autopus.json";
      mockReadConfigFileSnapshot.mockResolvedValueOnce(snapshot);

      await runConfigCommand(["config", "file"]);

      expect(mockLog).toHaveBeenCalledWith("/home/user/.autopus/autopus.json");
    });
  });
});
