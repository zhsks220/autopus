...
export function registerDnsCli(program: Command) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.autopus.ai/cli/dns")}\n`,
    );
  ...
  dns
    ...
    .option("--domain <domain>", t("opt.wide_area_discovery_domain_e_g_autopus_internal"))
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(
        theme.heading(
          "Recommended config ($AUTOPUS_CONFIG_PATH, default ~/.autopus/autopus.json):",
        ),
      );
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by autopus dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in the active Autopus config ($AUTOPUS_CONFIG_PATH, default ~/.autopus/autopus.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
