...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "autopus-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "autopus-bundled-skills-test-"));
    process.env.AUTOPUS_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
