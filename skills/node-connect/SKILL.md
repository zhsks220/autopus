---
name: node-connect
description: Diagnose Autopus Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Autopus is advertising that route, then fix pairing/auth.
...
- whether `autopus devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `autopus qr --json`. It uses the same setup-code payload Android scans.

```bash
autopus config get gateway.mode
autopus config get gateway.bind
autopus config get gateway.tailscale.mode
autopus config get gateway.remote.url
autopus config get gateway.auth.mode
autopus config get gateway.auth.allowTailscale
autopus config get plugins.entries.device-pair.config.publicUrl
autopus qr --json
autopus devices list
autopus nodes status
```

If this Autopus instance is pointed at a remote gateway, also run:

```bash
autopus qr --remote --json
```
...
If `autopus qr --json` says `Gateway is only bound to loopback`:
...
- `autopus devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run autopus qr again, rescan, then approve the pending device pairing.`
