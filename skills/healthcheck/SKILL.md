---
name: healthcheck
description: Audit and harden hosts running Autopus for SSH, firewall, updates, exposure, cron checks, and risk posture.
---

# Autopus Host Hardening

## Overview

Assess and harden the host running Autopus, then align it to a user-defined risk tolerance without breaking access. Use Autopus security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Autopus changes the host firewall, SSH, or OS updates; it does not.
...
5. Autopus gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Autopus can access sensitive data.
...
### 2) Run Autopus security audits (read-only)

As part of the default read-only checks, run `autopus security audit --deep`. Only offer alternatives if the user requests them:

1. `autopus security audit` (faster, non-probing)
2. `autopus security audit --json` (structured output)

Offer to apply Autopus safe defaults (numbered):

1. `autopus security audit --fix`

Be explicit that `--fix` only tightens Autopus defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Autopus version/update status (read-only)

As part of the default read-only checks, run `autopus update status`.
...
- Credential hygiene notes (location of Autopus creds, prefer disk encryption)
...
After Autopus install or first hardening pass, run at least one baseline audit and version check:

- `autopus security audit`
- `autopus security audit --deep`
- `autopus update status`

Ongoing monitoring is recommended. Use the Autopus cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `autopus cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `autopus update status`
...
Before creating, `autopus cron list` and match on exact `name`. If found, `autopus cron edit <id> ...`.
If not found, `autopus cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `autopus update status` (preferred for source checkouts and channels)
2. `npm view autopus version` (published npm version)

## Autopus command accuracy

Use only supported commands and flags:

- `autopus security audit [--deep] [--fix] [--json]`
- `autopus status` / `autopus status --deep`
- `autopus health --json`
- `autopus update status`
- `autopus cron add|list|runs|run`

Do not invent CLI flags or imply Autopus enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Autopus compaction:
