...
1. Download the runtime for your OS (extracts into `$AUTOPUS_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.autopus/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$AUTOPUS_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.autopus/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Autopus config file (`$AUTOPUS_CONFIG_PATH`, default `~/.autopus/autopus.json`):
