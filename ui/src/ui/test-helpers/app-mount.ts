...
import "../app.ts";
import type { AutopusApp } from "../app.ts";
...
const mountedApps = new Set<AutopusApp>();

function collectMountedApps() {
  return new Set<AutopusApp>([
    ...mountedApps,
    ...document.querySelectorAll<AutopusApp>("autopus-app"),
  ]);
}
...
async function waitForAppUpdates(apps: Iterable<AutopusApp>) {
  for (const app of apps) {
    await app.updateComplete;
  }
}

async function drainAppWork(apps: Iterable<AutopusApp>) {
  const snapshot = [...apps];
  await nextMicrotask();
...
export function mountApp(pathname: string) {
  ...
  const app = document.createElement("autopus-app") as AutopusApp;
  mountedApps.add(app);
...
export function registerAppMountHooks() {
  beforeEach(async () => {
    ...
    window.__AUTOPUS_CONTROL_UI_BASE_PATH__ = undefined;
