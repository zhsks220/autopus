...
// Prefix for Autopus-generated action IDs to scope our handler
const AUTOPUS_ACTION_PREFIX = "autopus:";
...
export function registerSlackInteractionEvents(params: {
  ...
  const modalMatcher = new RegExp(`^${AUTOPUS_ACTION_PREFIX}`);

  // Handle Autopus modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
