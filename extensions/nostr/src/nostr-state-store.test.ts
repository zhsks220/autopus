...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.AUTOPUS_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "autopus-nostr-"));
  process.env.AUTOPUS_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.AUTOPUS_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".autopus");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.AUTOPUS_STATE_DIR = previous;
    }
