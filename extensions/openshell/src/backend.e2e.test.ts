...
const AUTOPUS_OPENSHELL_E2E = process.env.AUTOPUS_E2E_OPENSHELL === "1";
const AUTOPUS_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const AUTOPUS_OPENSHELL_COMMAND = process.env.AUTOPUS_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "autopus-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && AUTOPUS_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: AUTOPUS_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(AUTOPUS_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(AUTOPUS_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "autopus-openshell-e2e-"));
      ...
      const gatewayName = `autopus-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `autopus-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/autopus-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: AUTOPUS_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("autopus-openshell-e2e");
