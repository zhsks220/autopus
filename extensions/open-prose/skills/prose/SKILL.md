...
metadata: { "autopus": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Autopus `sessions_spawn`
- **File I/O** == Autopus `read`/`write`
- **Remote fetch** == Autopus `web_fetch` (or `exec` with curl when POST is required)
