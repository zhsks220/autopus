...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__autopus_channel_id";
...
type InteractionCallbackConfig = Pick<AutopusConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "autopus-mattermost-interactions").update(botToken).digest("hex");
}
