...
let listNativeCommandSpecs: typeof import("autopus/plugin-sdk/command-auth").listNativeCommandSpecs;
...
function createNativeCommand(
  ...
  const baseCfg: AutopusConfig = opts?.cfg ?? {};
  const discordConfig: NonNullable<AutopusConfig["channels"]>["discord"] =
...
describe("createDiscordNativeCommand option wiring", () => {
  beforeAll(async () => {
    ({ listNativeCommandSpecs } = await import("autopus/plugin-sdk/command-auth"));
  ...
  it("returns no autocomplete choices for group DMs outside dm.groupChannels", async () => {
    ...
    } satisfies NonNullable<AutopusConfig["channels"]>["discord"];
    const command = createNativeCommand("think", {
      cfg: {
        commands: {
          allowFrom: {
  ...
  it("truncates Discord command and option descriptions to Discord's limit", () => {
    ...
    const discordConfig = {} as NonNullable<AutopusConfig["channels"]>["discord"];
