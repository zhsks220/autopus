...
import { isTruthyEnvValue, type RuntimeEnv } from "autopus/plugin-sdk/runtime-env";
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.autopusCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.autopusCanvasA2UIAction &&
        typeof window.autopusCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.autopusSendUserAction === "function";
  ...
  window.addEventListener("autopus:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Autopus node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.autopusSendUserAction === "function"
        ? window.autopusSendUserAction
        : undefined;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Autopus Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
