...
export const signalDmPolicy = {
  ...
  resolveConfigKeys: (cfg: AutopusConfig, accountId?: string) =>
...
export const signalCompletionNote = {
  ...
  lines: [
    'Link device with: signal-cli link -n "Autopus"',
    "Scan QR in Signal -> Linked Devices",
    `Then run: ${formatCliCommand("autopus gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
