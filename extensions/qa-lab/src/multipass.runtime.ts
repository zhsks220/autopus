...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/autopus-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/autopus";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/autopus-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `autopus-qa-${createVmSuffix()}`;
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "AUTOPUS_CONFIG_PATH" &&
          key !== "AUTOPUS_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `AUTOPUS_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `AUTOPUS_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm autopus qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredAutopusTmpDir(), `${plan.vmName}-qa-suite-`),
  );
