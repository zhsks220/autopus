...
const QA_LAB_UI_OVERLAY_DIR = "/opt/autopus-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        AUTOPUS_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    environment:
      AUTOPUS_CONFIG_PATH: /tmp/autopus/autopus.json
      ...
      AUTOPUS_PROFILE: ""
    volumes:
      - ./state:/opt/autopus-scaffold:ro
      - ${repoMount}:/opt/autopus-repo:ro
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/autopus/workspace /tmp/autopus/state && cp /opt/autopus-scaffold/autopus.json /tmp/autopus/autopus.json && cp -R /opt/autopus-scaffold/seed-workspace/. /tmp/autopus/workspace/ && ln -snf /opt/autopus-repo /tmp/autopus/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t autopus:qa-local-prebaked --build-arg AUTOPUS_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "autopus:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "autopus.json"),
  ];
