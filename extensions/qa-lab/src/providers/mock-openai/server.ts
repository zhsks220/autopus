...
import { readRequestBodyWithLimit } from "autopus/plugin-sdk/webhook-ingress";
...
function buildAssistantText(
  ...
  if (/tool continuity check/i.test(prompt) && toolOutput) {
    return `Protocol note: model switch handoff confirmed on ${model || "the requested model"}. QA mission from QA_KICKOFF_TASK.md still applies: understand this Autopus repo from source + docs before acting.`;
  }
  ...
  if (toolOutput && /octopus invaders/i.test(prompt)) {
    if (toolOutput.includes("QA mission") || toolOutput.includes("Testing")) {
      return "";
    }
    return `Protocol note: Octopus Invaders built at octopus-invaders.html.`;
  }
...
async function buildResponsesPayload(
  ...
  if (QA_TOOL_SEARCH_PROMPT_RE.test(allInputText) && !toolOutput) {
    ...
    if (targetTool && hasDeclaredTool(body, "tool_search_code")) {
      return buildToolCallEventsWithArgs("tool_search_code", {
        code: [
          `const hits = await autopus.tools.search(${JSON.stringify(targetTool)}, { limit: 1 });`,
          "const match = hits.find((tool) => tool.name === " + JSON.stringify(targetTool) + ");",
          "if (!match) throw new Error('target tool not found');",
          "return await autopus.tools.call(match.id, { marker: 'code-mode' });",
        ].join("\n"),
      });
    }
  ...
  if (/octopus invaders/i.test(prompt)) {
    if (!toolOutput) {
      return buildToolCallEventsWithArgs("read", { path: "QA_KICKOFF_TASK.md" });
    }
    if (toolOutput.includes("QA mission") || toolOutput.includes("Testing")) {
      return buildToolCallEventsWithArgs("write", {
        path: "octopus-invaders.html",
        content: `<!doctype html>
<html lang="en">
  <head><meta charset="utf-8" /><title>Octopus Invaders</title></head>
  <body><h1>Octopus Invaders</h1><p>Tiny playable stub.</p></body>
</html>`,
      });
    }
  }
