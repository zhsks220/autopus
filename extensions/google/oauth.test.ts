...
describe("extractGeminiCliCredentials", () => {
  ...
  function installWindowsNvmLayoutWithUnrelatedOauth(params: {
    ...
    const nvmRoot = join(rootDir, "fake", "Users", "octopus", "AppData", "Local", "nvm");
...
describe("loginGeminiCliOAuth", () => {
  ...
  function userInfoResponse(): Response {
    return responseJson({ email: "octopus@autopus.ai" });
  }
  ...
  beforeEach(() => {
    envSnapshot = Object.fromEntries(ENV_KEYS.map((key) => [key, process.env[key]]));
    process.env.AUTOPUS_GEMINI_OAUTH_CLIENT_ID = "test-client-id.apps.googleusercontent.com";
    process.env.AUTOPUS_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret"; // pragma: allowlist secret
