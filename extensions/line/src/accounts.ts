...
import { resolveAccountEntry } from "autopus/plugin-sdk/account-resolution";
import { tryReadSecretFileSync } from "autopus/plugin-sdk/core";
import type {
  LineAccountConfig,
  LineConfig,
  LineTokenSource,
...
export function listLineAccountIds(cfg: AutopusConfig): string[] {
  const lineConfig = cfg.channels?.line as LineConfig | undefined;
  const accounts = lineConfig?.accounts;
...
export function resolveDefaultLineAccountId(cfg: AutopusConfig): string {
