...
import { __testing as sessionBindingTesting } from "autopus/plugin-sdk/conversation-runtime";
import {
  createTestRegistry,
...
describe("buildLineMessageContext", () => {
  ...
  beforeEach(async () => {
    ...
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "autopus-line-context-"));
  ...
  it("group peer binding matches raw groupId without prefix (#21907)", async () => {
    ...
    const bindingCfg: AutopusConfig = {
