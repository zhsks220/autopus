...
Read-only diff viewer plugin for **Autopus** agents.
...
Before:
Autopus supports plugins.

After:
Autopus supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/autopus-diffs`).
