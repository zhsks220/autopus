...
export async function makeTempRoot() {
  const root = await fs.mkdtemp(
    path.join(resolvePreferredAutopusTmpDir(), "autopus-migrate-hermes-"),
  );
  tempRoots.add(root);
  return root;
}
...
export function makeConfigRuntime(
  ...
  onWrite?: (next: AutopusConfig) => void,
): NonNullable<MigrationProviderContext["runtime"]> {
  const commitConfig = (next: AutopusConfig) => {
    for (const key of Object.keys(config) as Array<keyof AutopusConfig>) {
      delete config[key];
    }
  ...
  return {
    config: {
      ...
      mutateConfigFile: async ({
        ...
        afterWrite?: unknown;
        mutate: (draft: AutopusConfig, context: unknown) => Promise<unknown> | void;
      }) => {
...
export function makeContext(params: {
  ...
  overwrite?: boolean;
  model?: NonNullable<NonNullable<AutopusConfig["agents"]>["defaults"]>["model"];
  reportDir?: string;
