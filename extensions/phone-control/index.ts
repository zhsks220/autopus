...
function normalizeDenyList(cfg: AutopusPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: AutopusPluginApi["config"]): string[] {
...
async function disarmNow(params: {
  ...
  const cfg = api.runtime.config.current() as AutopusConfig;
...
export default definePluginEntry({
  ...
  register(api: AutopusPluginApi) {
    ...
    const timerService: AutopusPluginService = {
