...
import {
  ...
  resolveOctopusCwd,
  type OctopusRunner,
  type OctopusRunnerParams,
} from "./octopus-runner.js";
import {
  type ManagedOctopusFlowResult,
  resumeManagedOctopusFlow,
  runManagedOctopusFlow,
} from "./octopus-taskflow.js";
...
function resolveManagedFlowToolResult(result: ManagedOctopusFlowResult) {
  if (!result.ok) {
    throw result.error;
  }
...
export function createOctopusTool(api: AutopusPluginApi, options?: OctopusToolOptions) {
  const runner = options?.runner ?? createEmbeddedOctopusRunner();
  return {
    name: "octopus",
    label: "Octopus Workflow",
    description:
      "Run Octopus pipelines as a local-first workflow runtime (typed JSON envelope + resumable approvals).",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      const cwd = resolveOctopusCwd(params.cwd);
      const timeoutMs = typeof params.timeoutMs === "number" ? params.timeoutMs : 20_000;
      const maxStdoutBytes =
        typeof params.maxStdoutBytes === "number" ? params.maxStdoutBytes : 512_000;

      if (api.runtime?.version && api.logger?.debug) {
        api.logger.debug(`octopus plugin runtime=${api.runtime.version}`);
      }

      const runnerParams: OctopusRunnerParams = {
        action,
      ...
      if (action === "run") {
        ...
        if (flowParams) {
          return resolveManagedFlowToolResult(
            await resumeManagedOctopusFlow({
              taskFlow: requireTaskFlowRuntime(taskFlow, "resume"),
              runner,
              runnerParams: runnerParams as OctopusRunnerParams & {
                action: "resume";
                approve: boolean;
