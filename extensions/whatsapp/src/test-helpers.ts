...
import { formatEnvelopeTimestamp } from "autopus/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("autopus:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("autopus:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[autopus]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/autopus-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("autopus:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("autopus:lastSocket")];
  if (typeof getter === "function") {
