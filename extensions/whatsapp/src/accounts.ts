...
import type { DmPolicy, GroupPolicy, ReplyToMode } from "autopus/plugin-sdk/config-contracts";
import { resolveOAuthDir } from "autopus/plugin-sdk/state-paths";
...
export function listWhatsAppAuthDirs(cfg: AutopusConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: AutopusConfig): boolean {
...
export function resolveWhatsAppAuthDir(params: { cfg: AutopusConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: AutopusConfig): ResolvedWhatsAppAccount[] {
