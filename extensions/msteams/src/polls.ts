...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["autopusPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["autopus", "pollId"]) ??
    readNestedString(value, ["autopus", "poll", "id"]) ??
    readNestedString(value, ["data", "autopusPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "autopus", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          autopusPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "autopus poll vote",
          displayText: "Vote recorded",
          value: { autopusPollId: pollId, pollId },
        },
      },
    ],
  };
