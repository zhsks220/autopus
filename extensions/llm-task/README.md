...
Designed to be called from workflow engines (for example, Octopus via
`autopus.invoke --each`) without adding new Autopus code per workflow.
...
- Side effects should be handled outside this tool (for example, approvals in
  Octopus) before calling tools that send messages/emails.

## Bundled extension note

This extension depends on Autopus internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Autopus extension (like `octopus`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.autopus/extensions` as a standalone plugin directory.
