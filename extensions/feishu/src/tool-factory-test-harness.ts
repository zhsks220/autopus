...
export function createToolFactoryHarness(cfg: AutopusPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<AutopusPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
