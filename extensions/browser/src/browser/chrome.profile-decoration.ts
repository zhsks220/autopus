...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".autopus-profile-decorated");
}
 ...
 */
export function decorateAutopusProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_AUTOPUS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_AUTOPUS_BROWSER_COLOR).toUpperCase();
