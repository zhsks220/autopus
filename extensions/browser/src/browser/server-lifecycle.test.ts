...
const { stopAutopusChromeMock } = vi.hoisted(() => ({
  stopAutopusChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopAutopusChrome: stopAutopusChromeMock,
}));
...
beforeEach(() => {
  ...
  stopAutopusChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["autopus", "user"]);
    ...
    expect(stopMap.autopus).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopAutopusChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("autopus browser stop failed: Error: oops");
  });
});
