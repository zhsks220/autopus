...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        autopus: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.AUTOPUS_GATEWAY_PORT;
    process.env.AUTOPUS_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.AUTOPUS_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.AUTOPUS_GATEWAY_PASSWORD;
