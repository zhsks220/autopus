...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleAutopusDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Autopus devices detected for ${params.auth.userId}: ${deviceHealth.staleAutopusDevices.map((device) => device.deviceId).join(", ")}. Run 'autopus matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
