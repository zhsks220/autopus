...
function createDeps(
  ...
  return {
    ...
    summarizeMatrixDeviceHealth: vi.fn(() => ({
      ...
      staleAutopusDevices: [] as MatrixManagedDeviceInfo[],
      currentAutopusDevices: [] as MatrixManagedDeviceInfo[],
    })),
...
describe("runMatrixStartupMaintenance", () => {
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    vi.mocked(deps.summarizeMatrixDeviceHealth).mockReturnValue({
      currentDeviceId: null,
      staleAutopusDevices: [{ deviceId: "DEV123", displayName: "Autopus Device", current: false }],
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale Autopus devices detected for @bot:example.org: DEV123. Run 'autopus matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
      "matrix: device not verified — run 'autopus matrix verify device <key>' to enable E2EE",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
