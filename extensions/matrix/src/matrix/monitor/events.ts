...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'autopus matrix verify status --verbose --account ${accountId}' and 'autopus matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Autopus runtime is using the same account. " +
    `Check 'autopus matrix verify status --verbose --account ${accountId}' and 'autopus matrix devices list --account ${accountId}'.`
  );
}
