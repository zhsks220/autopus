...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "autopus-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<AutopusConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<AutopusConfig["channels"]>["matrix"]>>,
): AutopusConfig {
  return {
    channels: {
      matrix: {
