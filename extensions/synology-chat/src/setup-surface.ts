...
function getChannelConfig(cfg: AutopusConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: AutopusConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: AutopusConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: AutopusConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Autopus uses to send replies back to Chat.",
      ],
