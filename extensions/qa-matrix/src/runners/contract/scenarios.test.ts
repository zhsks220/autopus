...
const {
  ...
  formatMatrixQaCliCommand: (args: string[]) => `autopus ${args.join(" ")}`,
  redactMatrixQaCliOutput: (text: string) => text,
  resolveMatrixQaAutopusCliEntryPath: (cwd: string) => `${cwd}/dist/index.js`,
  runMatrixQaAutopusCli: vi.fn(),
  startMatrixQaAutopusCli: vi.fn(),
}));
...
describe("matrix live qa scenarios", () => {
  beforeEach(() => {
    ...
    runMatrixQaAutopusCli.mockReset();
    startMatrixQaAutopusCli.mockReset();
  ...
  it("merges default and scenario-requested Matrix topology once per run", () => {
    expect(
      scenarioTesting.buildMatrixQaTopologyForScenarios({
        defaultRoomName: "Autopus Matrix QA run",
        scenarios: [
  ...
  it("configures a fresh encrypted room before sync-state-loss recovery", async () => {
    ...
    try {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        gatewayRuntimeEnv: {
          AUTOPUS_CONFIG_PATH: gatewayConfigPath,
  ...
  it("runs Matrix self-verification through the interactive CLI command", async () => {
    ...
    try {
      ...
      startMatrixQaAutopusCli.mockReturnValue({
        args: ["matrix", "verify", "self", "--account", "cli"],
        endStdin,
        kill,
      ...
      runMatrixQaAutopusCli.mockImplementation(async ({ args, env, stdin }) => {
        if (!cliAccountConfigDuringRun && env.AUTOPUS_CONFIG_PATH) {
          const cliConfig = JSON.parse(await readFile(String(env.AUTOPUS_CONFIG_PATH), "utf8")) as {
            channels?: {
              matrix?: {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverPassword: "driver-password",
        gatewayRuntimeEnv: {
          AUTOPUS_CONFIG_PATH: "/tmp/gateway-config.json",
          AUTOPUS_STATE_DIR: "/tmp/gateway-state",
          PATH: process.env.PATH,
        },
        outputDir,
      });
      ...
      expect(startMatrixQaAutopusCli).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(startMatrixQaAutopusCli, "startMatrixQaAutopusCli").args).toEqual([
        "matrix",
        "verify",
        "self",
        "--account",
      ...
      expect(mockObjectArg(startMatrixQaAutopusCli, "startMatrixQaAutopusCli").timeoutMs).toBe(
        16_000,
      );
      ...
      expect(registrationRequest?.deviceName).toBe("Autopus Matrix QA CLI Self Verification Owner");
      if (
      ...
      expect(runMatrixQaAutopusCli).toHaveBeenCalledTimes(2);
      expect(runMatrixQaAutopusCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          "matrix",
          "verify",
          "backup",
      ...
      expect(mockObjectArg(runMatrixQaAutopusCli, "runMatrixQaAutopusCli").stdin).toBe(
        "encoded-recovery-key\n",
      );
      const cliEnv = mockObjectArg(startMatrixQaAutopusCli, "startMatrixQaAutopusCli")
        .env as Record<string, unknown>;
      expect(cliEnv?.AUTOPUS_STATE_DIR).toContain("autopus-matrix-cli-qa-");
      expect(cliEnv?.AUTOPUS_CONFIG_PATH).toContain("autopus-matrix-cli-qa-");
      const configPath = String(cliEnv?.AUTOPUS_CONFIG_PATH);
      ...
      await expectPathMissing(String(cliEnv?.AUTOPUS_STATE_DIR));
  ...
  it("runs Matrix account add --enable-e2ee through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaAutopusCli.mockImplementation(async ({ args, env }) => {
        if (env.AUTOPUS_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.AUTOPUS_CONFIG_PATH), "utf8"),
          ) as {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Autopus Matrix QA CLI Account Add Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaAutopusCli.mockImplementation(async ({ args, env }) => {
        if (!initialAccountConfig && env.AUTOPUS_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.AUTOPUS_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Autopus Matrix QA CLI Encryption Setup Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup idempotency through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Autopus Matrix QA CLI Encryption Idempotent Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup bootstrap failure through the CLI QA scenario", async () => {
    ...
    try {
      ...
      const wait = vi.fn().mockRejectedValue(new Error("autopus matrix encryption setup exited 1"));
      const kill = vi.fn();
      startMatrixQaAutopusCli.mockReturnValue({
      ...
      expect(
        (
          mockObjectArg(startMatrixQaAutopusCli, "startMatrixQaAutopusCli").env as Record<
            string,
            unknown
          >
        ).AUTOPUS_CONFIG_PATH,
      ).toContain("autopus-matrix-e2ee-setup-qa-");
      expect(output).toHaveBeenCalledTimes(1);
      expect(wait).toHaveBeenCalledTimes(1);
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Autopus Matrix QA CLI Encryption Failure Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Autopus Matrix QA CLI Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix invalid recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Autopus Matrix QA CLI Invalid Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix multi-account encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaAutopusCli.mockImplementation(async ({ args, env }) => {
        const configPath = String(env.AUTOPUS_CONFIG_PATH);
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Autopus Matrix QA CLI Multi Account Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
    ...
    try {
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "Autopus Matrix QA CLI Setup Gateway" &&
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "Autopus Matrix QA CLI Setup Driver" &&
            request.registrationToken === "registration-token",
        ),
      ).toBe(true);
      expect(createPrivateRoom).toHaveBeenCalledWith({
