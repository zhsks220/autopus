...
function resolveQianfanPreset(cfg: AutopusConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: AutopusConfig) => {
...
export function applyQianfanProviderConfig(cfg: AutopusConfig): AutopusConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: AutopusConfig): AutopusConfig {
