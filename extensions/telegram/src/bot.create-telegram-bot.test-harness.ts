...
type GetRuntimeConfigFn =
  typeof import("autopus/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;
type LoadSessionStoreFn =
  typeof import("autopus/plugin-sdk/session-store-runtime").loadSessionStore;
type ResolveStorePathFn =
  typeof import("autopus/plugin-sdk/session-store-runtime").resolveStorePath;
type ReadSessionUpdatedAtFn =
  typeof import("autopus/plugin-sdk/session-store-runtime").readSessionUpdatedAt;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/autopus-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
function resolveDefaultModelForAgentForTest(params: { cfg: AutopusConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: AutopusConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: AutopusConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: AutopusConfig) => {
