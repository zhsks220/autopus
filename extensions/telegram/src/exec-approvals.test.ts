...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "autopus-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<AutopusConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<AutopusConfig["channels"]>["telegram"]>>,
): AutopusConfig {
  return {
    channels: {
      telegram: {
