...
import { defaultToolStreamExtraParams } from "autopus/plugin-sdk/provider-stream-shared";
import { jsonResult } from "autopus/plugin-sdk/provider-web-search";
    ...
    execute: async (toolCallId: string, args: Record<string, unknown>) => {
      ...
      if (!tool) {
        return jsonResult({
          ...
          message:
            "code_execution needs an xAI API key. Run autopus onboard --auth-choice xai-api-key, set XAI_API_KEY in the Gateway environment, or configure plugins.entries.xai.config.webSearch.apiKey.",
