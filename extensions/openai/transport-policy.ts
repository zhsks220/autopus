...
function resolveSessionHeaders(params: {
  ...
  return {
    ...
    "x-autopus-session-id": sessionId,
  };
}
...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-autopus-turn-id": turnId,
      "x-autopus-turn-attempt": attempt,
    },
    metadata: {
      autopus_session_id: sessionHeaders["x-autopus-session-id"] ?? "",
      autopus_turn_id: turnId,
      autopus_turn_attempt: attempt,
      autopus_transport: ctx.transport,
    },
  };
}
