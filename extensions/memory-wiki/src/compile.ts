...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- autopus:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- autopus:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        ...
        relativePath,
        title: group.heading,
        startMarker: `<!-- autopus:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- autopus:wiki:${group.dir}:index:end -->`,
