...
- Start from `autopus wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `autopus wiki obsidian status` before shelling out, then prefer the dedicated helpers like `autopus wiki obsidian search`, `autopus wiki obsidian open`, `autopus wiki obsidian command`, and `autopus wiki obsidian daily`.
