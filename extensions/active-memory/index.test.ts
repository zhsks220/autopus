...
describe("active-memory plugin", () => {
  ...
  const api: any = {
    ...
    runtime: {
      agent: {
        runEmbeddedPiAgent,
        session: {
          resolveStorePath: vi.fn(() => "/tmp/autopus-session-store.json"),
  ...
  beforeEach(async () => {
    ...
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "autopus-active-memory-test-"));
    configFile = {
      plugins: {
        entries: {
          "active-memory": {
    ...
    plugin.register(api as unknown as AutopusPluginApi);
  });
  ...
  it("skips missing memory tools when the allowlist error includes inherited sources", async () => {
    ...
    const error = makeMemoryToolAllowlistError(
      ...
      "tools.allow: *, octopus; runtime toolsAllow: memory_search, memory_get",
    );
    expect(__testing.isMissingRegisteredMemoryToolsError(error)).toBe(true);
    runEmbeddedPiAgent.mockRejectedValueOnce(error);
  ...
  it("keeps subagent transcripts off disk by default by using a temp session file", async () => {
    ...
    expect(sessionFile).toMatch(/autopus-active-memory-.*\/session\.jsonl$/);
