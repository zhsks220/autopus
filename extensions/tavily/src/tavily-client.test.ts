...
vi.mock("autopus/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: autopus", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "autopus" });
  });

  it("runTavilyExtract sends X-Client-Source: autopus", async () => {
