...
vi.mock("autopus/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("autopus/plugin-sdk/memory-core-host-engine-foundation")
  >("autopus/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
import { resolveMemoryBackendConfig } from "autopus/plugin-sdk/memory-core-host-engine-storage";
...
describe("QmdMemoryManager slugified path resolution", () => {
  ...
  async function createManager(params?: { cfg?: AutopusConfig }) {
  ...
  beforeEach(async () => {
    ...
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autopus-qmd-slugified-"));
