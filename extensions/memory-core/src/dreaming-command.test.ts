...
function resolveStoredDreaming(config: AutopusConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: AutopusConfig = {}) {
  const registered: { command?: AutopusPluginCommandDefinition } = {};
  let runtimeConfig: AutopusConfig = initialConfig;
  ...
  const runtime = {
    config: {
      ...
      writeConfigFile: vi.fn(async (nextConfig: AutopusConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: AutopusPluginCommandDefinition) => {
