...
import type { MemoryEmbeddingProbeResult } from "autopus/plugin-sdk/memory-core-host-engine-storage";
import { resolveMemoryRemDreamingConfig } from "autopus/plugin-sdk/memory-core-host-status";
...
function resolveMemoryPluginConfig(cfg: AutopusConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredAutopusTmpDir(), "autopus-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: AutopusConfig): string {
...
function resolveAgent(cfg: AutopusConfig, agent?: string) {
...
function resolveAgentIds(cfg: AutopusConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: autopus memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredAutopusTmpDir(), "autopus-rem-backfill-"),
      );
      try {
