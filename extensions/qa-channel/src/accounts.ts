...
import { resolveMergedAccountConfig } from "autopus/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "autopus";
  const botDisplayName = merged.botDisplayName?.trim() || "Autopus QA";
  return {
    accountId,
    enabled,
