...
type DriverMode = "token" | "webhook" | "autopus";
...
function resolveStateDir(): string {
  const override = process.env.AUTOPUS_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.AUTOPUS_HOME?.trim() || process.env.HOME || "";
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver autopus] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Autopus spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|autopus> Driver transport mode (default: token)\n" +
    ...
    "  --autopus-bin <path>        Autopus CLI binary for driver=autopus (default: autopus)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  AUTOPUS_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  AUTOPUS_DISCORD_SMOKE_DRIVER\n" +
    "  AUTOPUS_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  AUTOPUS_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  AUTOPUS_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  AUTOPUS_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  AUTOPUS_DISCORD_SMOKE_AGENT\n" +
    "  AUTOPUS_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  AUTOPUS_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  AUTOPUS_DISCORD_SMOKE_POLL_MS\n" +
    "  AUTOPUS_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  AUTOPUS_DISCORD_SMOKE_AUTOPUS_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.AUTOPUS_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.AUTOPUS_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "autopus"
        ? "autopus"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken = resolveArg("--token") || process.env.AUTOPUS_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.AUTOPUS_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.AUTOPUS_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.AUTOPUS_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent = resolveArg("--agent") || process.env.AUTOPUS_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.AUTOPUS_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.AUTOPUS_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.AUTOPUS_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.AUTOPUS_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.AUTOPUS_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const autopusBin =
    resolveArg("--autopus-bin") || process.env.AUTOPUS_DISCORD_SMOKE_AUTOPUS_BIN || "autopus";
...
async function autopusCliJson<T>(params: { autopusBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.autopusBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`autopus ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithAutopus(params: {
  autopusBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await autopusCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    autopusBin: params.autopusBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "autopus") {
    return await readMessagesWithAutopus({
      autopusBin: params.args.autopusBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `autopus-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await autopusCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("autopus message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "autopus"
            ? await readMessagesWithAutopus({
                autopusBin: args.autopusBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Autopus.`,
        diagnostics: {
          bindingCandidates: [
            {
