...
const PLUGIN_SPEC = process.env.AUTOPUS_KITCHEN_SINK_NPM_SPEC || "npm:@autopus/kitchen-sink@latest";
const PLUGIN_ID = process.env.AUTOPUS_KITCHEN_SINK_PLUGIN_ID || "autopus-kitchen-sink-fixture";
...
const READY_TIMEOUT_MS = readPositiveInt(process.env.AUTOPUS_KITCHEN_SINK_RPC_READY_MS, 240000);
const COMMAND_TIMEOUT_MS = readPositiveInt(process.env.AUTOPUS_KITCHEN_SINK_RPC_COMMAND_MS, 180000);
const RPC_TIMEOUT_MS = readPositiveInt(process.env.AUTOPUS_KITCHEN_SINK_RPC_CALL_MS, 60000);
const MAX_RSS_MIB = readPositiveInt(process.env.AUTOPUS_KITCHEN_SINK_MAX_RSS_MIB, 2048);
...
function resolveAutopusRunner() {
  if (process.env.AUTOPUS_ENTRY) {
    return {
      command: "node",
      baseArgs: [process.env.AUTOPUS_ENTRY],
      label: process.env.AUTOPUS_ENTRY,
    };
  }
  ...
  return { command: "pnpm", baseArgs: ["autopus"], label: "pnpm autopus" };
}

function makeEnv() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "autopus-kitchen-sink-rpc-"));
  ...
  return {
    ...
    env: {
      ...
      AUTOPUS_KITCHEN_SINK_PERSONALITY:
        process.env.AUTOPUS_KITCHEN_SINK_PERSONALITY || "conformance",
    },
  };
}
...
async function runAutopus(runner, args, env, options = {}) {
...
async function rpcCall(method, params, options) {
  ...
  const payload = await callGateway({
    config: readJson(options.env.AUTOPUS_CONFIG_PATH),
    configPath: options.env.AUTOPUS_CONFIG_PATH,
    ...
    entries: {
      ...
      [PLUGIN_ID]: {
        ...
        config: {
          ...
          personality: env.AUTOPUS_KITCHEN_SINK_PERSONALITY,
        },
        hooks: {
...
async function main() {
  const runner = resolveAutopusRunner();
  const port = readPositiveInt(process.env.AUTOPUS_KITCHEN_SINK_RPC_PORT, 19173);
  const { root, env } = makeEnv();
  const logPath = path.join(root, "gateway.log");

  console.log(`Kitchen Sink RPC walk using ${PLUGIN_SPEC} via ${runner.label}`);
  await runAutopus(runner, ["plugins", "install", PLUGIN_SPEC], env, { timeoutMs: 240000 });
  configureKitchenSink(env, port);
  await runAutopus(runner, ["plugins", "enable", PLUGIN_ID], env, { timeoutMs: 60000 });
  const inspect = parseJsonOutput(
    (await runAutopus(runner, ["plugins", "inspect", PLUGIN_ID, "--runtime", "--json"], env))
      .stdout,
  );
