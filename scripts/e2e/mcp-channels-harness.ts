...
// against the Autopus npm tarball installed in the functional image.
...
export async function connectMcpClient(params: {
  ...
  const tokenDir = "/tmp/autopus-mcp-client";
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      AUTOPUS_STATE_DIR: "/tmp/autopus-mcp-client",
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[autopus mcp] ${String(chunk)}`);
  });
