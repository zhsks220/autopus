...
function getInstallRecords() {
  ...
  const allowLegacyCompat = process.env.AUTOPUS_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function assertPluginRemoved(params) {
  ...
  const config = readAutopusConfig();
...
function recordFixturePluginTrust() {
  ...
  const ledgerPath = path.join(process.env.HOME, ".autopus", "plugins", "installs.json");
  const ledger = fs.existsSync(ledgerPath)
    ? readJson(ledgerPath)
    : {
        version: 1,
        warning:
          "DO NOT EDIT. This file is generated by Autopus plugin install/update/uninstall commands. Use `autopus plugins install/update/uninstall` instead.",
        records: {},
      };
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".autopus", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(autopusPeerPath)) {
    throw new Error(`missing ClawHub autopus peer symlink: ${autopusPeerPath}`);
  }
  if (!fs.lstatSync(autopusPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub autopus peer is not a symlink: ${autopusPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(autopusPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub autopus peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@autopus/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@autopus/demo-plugin-npm") {
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.AUTOPUS_CLAWHUB_URL ||
    process.env.CLAWHUB_URL ||
    "https://clawhub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.AUTOPUS_CLAWHUB_TOKEN ||
...
function assertClawHubInstalled() {
  ...
  const installPath = record.installPath.replace(/^~(?=$|\/)/u, process.env.HOME);
  const extensionsRoot = path.join(process.env.HOME, ".autopus", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".autopus", "autopus.json");
