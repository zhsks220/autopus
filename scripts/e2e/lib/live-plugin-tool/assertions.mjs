...
const agentTurnTimeoutSeconds = Number.parseInt(
  process.env.AUTOPUS_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS ?? "300",
  10,
);
...
function pluginInstallPath() {
  ...
  const inspect = fs.existsSync("/tmp/autopus-plugin-inspect.json")
    ? readJson("/tmp/autopus-plugin-inspect.json")
    : {};
...
function writeFixture() {
  ...
  writeJson(path.join(dir, "autopus.plugin.json"), {
    id: pluginId,
    name: "E2E Slug Tool",
...
function assertInstalled() {
  ...
  const inspect = readJson("/tmp/autopus-plugin-inspect.json");
...
function assertAgentTurn() {
  ...
  const toolName = requireEnv("TOOL_NAME");
  const stdout = fs.readFileSync("/tmp/autopus-agent.json", "utf8");
  const stderr = fs.existsSync("/tmp/autopus-agent.err")
    ? fs.readFileSync("/tmp/autopus-agent.err", "utf8")
    : "";
