#!/bin/bash
# Setup Autopus Auth Management System
...
echo "=== Autopus Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'autopus-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Autopus message: Send warning via Autopus itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/autopus-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/autopus-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/autopus-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now autopus-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status autopus-auth-monitor.timer"
echo ""
echo "Quick commands:"
