...
function shouldBuildBundledDistEntry(packageJson) {
  return packageJson?.autopus?.build?.bundledDist !== false;
}

export function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.autopus?.extensions)
    ? packageJson.autopus.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.autopus?.setupEntry === "string" &&
    packageJson.autopus.setupEntry.trim().length > 0
      ? packageJson.autopus.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "autopus.plugin.json"));
    }
    if (hasPackageJson) {
