...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const autopusMeta =
    ...
    "autopus" in source.packageJson
      ? (source.packageJson.autopus as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    autopusMeta &&
    typeof autopusMeta.channel === "object" &&
    autopusMeta.channel &&
    !Array.isArray(autopusMeta.channel)
      ? (autopusMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
