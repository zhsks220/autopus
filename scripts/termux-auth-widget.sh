...
# Autopus Auth Widget for Termux
...
SERVER="${AUTOPUS_SERVER:-l36}"

# Check auth status
termux-toast "Checking Autopus auth..."

STATUS=$(ssh "$SERVER" '$HOME/autopus/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/autopus/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|AUTOPUS_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/autopus/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|AUTOPUS_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Autopus won't work until re-auth"
                ;;
        esac
        ;;
