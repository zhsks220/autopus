...
# Build and bundle Autopus into a minimal .app we can open.
# Outputs to dist/Autopus.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Autopus.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Autopus"
MLX_TTS_HELPER_PRODUCT="autopus-mlx-tts"
MLX_TTS_HELPER_ROOT="$ROOT_DIR/apps/macos-mlx-tts"
MLX_TTS_HELPER_BUILD_ROOT="$MLX_TTS_HELPER_ROOT/.build"
BUNDLE_ID="${BUNDLE_ID:-ai.autopus.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/autopus/autopus/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Autopus/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AutopusBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AutopusGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Autopus"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Autopus"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Autopus"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Autopus" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Autopus/Resources/Autopus.icns" "$APP_ROOT/Contents/Resources/Autopus.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Autopus/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying AutopusKit resources"
AUTOPUSKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AutopusKit_AutopusKit.bundle"
if [ -d "$AUTOPUSKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AutopusKit_AutopusKit.bundle"
  cp -R "$AUTOPUSKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AutopusKit_AutopusKit.bundle"
else
  echo "WARN: AutopusKit resource bundle not found at $AUTOPUSKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Autopus"
killall -q Autopus 2>/dev/null || true
