...
- regenerates apps/ios/Autopus.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${AUTOPUS_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid AUTOPUS_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid AUTOPUS_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid AUTOPUS_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid AUTOPUS_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing AUTOPUS_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(AUTOPUS_URL_SLASH)$(AUTOPUS_URL_SLASH)#g'
)"
...
AUTOPUS_CODE_SIGN_STYLE = Automatic
AUTOPUS_DEVELOPMENT_TEAM = ${TEAM_ID}
AUTOPUS_IOS_SELECTED_TEAM = ${TEAM_ID}
AUTOPUS_APP_BUNDLE_ID = ai.autopus.client
AUTOPUS_SHARE_BUNDLE_ID = ai.autopus.client.share
AUTOPUS_ACTIVITY_WIDGET_BUNDLE_ID = ai.autopus.client.activitywidget
AUTOPUS_WATCH_APP_BUNDLE_ID = ai.autopus.client.watchkitapp
AUTOPUS_WATCH_EXTENSION_BUNDLE_ID = ai.autopus.client.watchkitapp.extension
AUTOPUS_APP_PROFILE =
AUTOPUS_SHARE_PROFILE =
AUTOPUS_PUSH_TRANSPORT = relay
AUTOPUS_PUSH_DISTRIBUTION = official
AUTOPUS_URL_SLASH = /
AUTOPUS_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
AUTOPUS_PUSH_APNS_ENVIRONMENT = production
EOF
