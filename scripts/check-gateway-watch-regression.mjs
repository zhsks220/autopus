...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  AUTOPUS_SKIP_ACPX_RUNTIME: "1",
  AUTOPUS_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$AUTOPUS_WATCH_PID_FILE"',
    'mkdir -p "$AUTOPUS_HOME/.autopus"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$AUTOPUS_HOME/.autopus/autopus.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    AUTOPUS_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    AUTOPUS_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "autopus-gateway-watch-"));
