 ...
 * `autopus update`. Use it to verify the completion installation flow
...
\`autopus update\`, \`autopus doctor\`, and \`autopus onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Autopus Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`autopus completion --install\` later to enable.`));
    return;
  }
