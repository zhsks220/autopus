...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/autopus/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Autopus Re-Auth" -c "After getting key, run: ssh $SERVER '~/autopus/scripts/mobile-reauth.sh'" --id autopus-auth
        ;;
    *)
