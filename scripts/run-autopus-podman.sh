#!/usr/bin/env bash
# Rootless Autopus in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-autopus-podman.sh launch        # Start gateway
#   ./scripts/run-autopus-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   autopus --container autopus dashboard --no-open
#   autopus --container autopus channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      AUTOPUS_GATEWAY_TOKEN|AUTOPUS_PODMAN_CONTAINER|AUTOPUS_PODMAN_IMAGE|AUTOPUS_IMAGE|AUTOPUS_PODMAN_PULL|AUTOPUS_PODMAN_GATEWAY_HOST_PORT|AUTOPUS_GATEWAY_PORT|AUTOPUS_PODMAN_BRIDGE_HOST_PORT|AUTOPUS_BRIDGE_PORT|AUTOPUS_GATEWAY_BIND|AUTOPUS_PODMAN_USERNS|AUTOPUS_BIND_MOUNT_OPTIONS|AUTOPUS_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-autopus-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${AUTOPUS_CONFIG_DIR:-$EFFECTIVE_HOME/.autopus}"
ENV_FILE="${AUTOPUS_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${AUTOPUS_CONFIG_DIR:-$EFFECTIVE_HOME/.autopus}"
ENV_FILE="${AUTOPUS_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${AUTOPUS_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${AUTOPUS_PODMAN_CONTAINER:-autopus}"
AUTOPUS_IMAGE="${AUTOPUS_PODMAN_IMAGE:-${AUTOPUS_IMAGE:-autopus:local}}"
PODMAN_PULL="${AUTOPUS_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${AUTOPUS_PODMAN_GATEWAY_HOST_PORT:-${AUTOPUS_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${AUTOPUS_PODMAN_BRIDGE_HOST_PORT:-${AUTOPUS_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${AUTOPUS_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$AUTOPUS_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  AUTOPUS_CONTAINER="" AUTOPUS_CONFIG_DIR="$config_dir" \
    autopus config get gateway.bind 2>/dev/null || true
}
...
# AUTOPUS_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${AUTOPUS_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'AUTOPUS_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v autopus >/dev/null 2>&1; then
    echo "Warning: autopus not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    AUTOPUS_CONTAINER="" AUTOPUS_CONFIG_DIR="$config_dir" \
      autopus config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    AUTOPUS_CONTAINER="" AUTOPUS_CONFIG_DIR="$config_dir" \
      autopus config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  AUTOPUS_CONTAINER="" AUTOPUS_CONFIG_DIR="$config_dir" \
    autopus config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${AUTOPUS_GATEWAY_TOKEN:-}" ]]; then
  export AUTOPUS_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "AUTOPUS_GATEWAY_TOKEN" "$AUTOPUS_GATEWAY_TOKEN"
  echo "Generated AUTOPUS_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/autopus.json"
...
PODMAN_USERNS="${AUTOPUS_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported AUTOPUS_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (AUTOPUS_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${AUTOPUS_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${AUTOPUS_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$AUTOPUS_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.autopus/.npm \
    -e AUTOPUS_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.autopus:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.autopus/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$AUTOPUS_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME autopus dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME autopus devices approve --latest  # if pairing required"
