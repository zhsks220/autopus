#!/usr/bin/env bash
# ClawDock - Docker helpers for Autopus
# Inspired by Simon Willison's "Running Autopus in Docker"
# https://til.simonwillison.net/llms/autopus-docker
#
# Installation:
...
# Common paths to check for Autopus
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/autopus"
  "${HOME}/workspace/autopus"
  "${HOME}/projects/autopus"
  "${HOME}/dev/autopus"
  "${HOME}/code/autopus"
  "${HOME}/src/autopus"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🐙 Found Autopus at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/autopus"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Autopus not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/autopus/autopus.git ~/autopus"
    echo "  cd ~/autopus && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^AUTOPUS_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d autopus-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart autopus-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f autopus-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.autopus"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show autopus.json
  if [[ -f "${config_dir}/autopus.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/autopus.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/autopus.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No autopus.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec autopus-gateway \
    bash -c 'echo "alias autopus=\"./autopus.mjs\"" > /tmp/.bashrc_autopus && bash --rcfile /tmp/.bashrc_autopus'
}
...
clawdock-cli() {
  _clawdock_compose run --rm autopus-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build autopus-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d autopus-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build autopus-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "AUTOPUS_GATEWAY_TOKEN=$token" autopus-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" autopus-gateway \
    bash -c './autopus.mjs config set gateway.remote.token "$TOKEN" && ./autopus.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec autopus-gateway \
    bash -c "./autopus.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart autopus-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm autopus-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec autopus-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'autopus config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🐙 ClawDock - Docker Helpers for Autopus${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (autopus alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to autopus project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.autopus)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'autopus channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'autopus status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.autopus.ai${_CLR_RESET}"
  echo ""
}
