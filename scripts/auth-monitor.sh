...
#
# Suggested cron: */30 * * * * /home/admin/autopus/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Autopus notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., autopus-alerts)
...
STATE_FILE="$HOME/.autopus/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Autopus if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use autopus
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Autopus to $NOTIFY_PHONE..."
            autopus send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Autopus is down. Run: ssh l36 '~/autopus/scripts/mobile-reauth.sh'" "urgent"
    exit 1
