...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.AUTOPUS_BUILD_PRIVATE_QA === "1";
}
...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@autopus/${params.dirName}`) {
    return false;
  }
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "autopus.plugin.json");
    ...
    if (packageJson.autopus && "extensions" in packageJson.autopus) {
      packageJson.autopus = {
        ...packageJson.autopus,
        extensions: rewritePackageExtensions(packageJson.autopus.extensions),
        ...(typeof packageJson.autopus.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.autopus.setupEntry) }
          : {}),
      };
    }
