...
export function resolveVitestNodeArgs(env = process.env) {
  if (isTruthyEnvValue(env.AUTOPUS_VITEST_ENABLE_MAGLEV)) {
    return [];
  }
...
export function resolveVitestNoOutputTimeoutMs(env = process.env) {
  return parsePositiveInt(env.AUTOPUS_VITEST_NO_OUTPUT_TIMEOUT_MS);
}
...
function shouldApplyNativeWorkerBudget(env) {
  ...
  return (
    env.AUTOPUS_TEST_PROJECTS_SERIAL === "1" || resolveExplicitVitestWorkerBudget(env) !== null
  );
}
...
function resolveExplicitVitestWorkerBudget(env) {
  return parsePositiveInt(env.AUTOPUS_VITEST_MAX_WORKERS ?? env.AUTOPUS_TEST_WORKERS);
}
