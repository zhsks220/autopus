...
# Autopus Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://autopus.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Autopus."
...
GUM_VERSION="${AUTOPUS_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🐙 Autopus Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://autopus.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$AUTOPUS_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.autopus.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_autopus_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.autopus-* "$npm_root"/autopus 2>/dev/null || true
}

extract_autopus_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_autopus_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/autopus" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/autopus"|"/usr/local/bin/autopus")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/autopus/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale autopus symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing autopus binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Autopus package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_autopus_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Autopus npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*autopus" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_autopus_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_autopus_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Autopus npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an autopus binary already exists"
            if [[ -n "$conflict" ]]; then
...
TAGLINES+=("The only octopus in your contacts you actually want to hear from. 🐙")
...
pick_tagline() {
    ...
    if [[ -n "${AUTOPUS_TAGLINE_INDEX:-}" ]]; then
        if [[ "${AUTOPUS_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((AUTOPUS_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${AUTOPUS_NO_ONBOARD:-0}
NO_PROMPT=${AUTOPUS_NO_PROMPT:-0}
DRY_RUN=${AUTOPUS_DRY_RUN:-0}
INSTALL_METHOD=${AUTOPUS_INSTALL_METHOD:-}
AUTOPUS_VERSION=${AUTOPUS_VERSION:-latest}
USE_BETA=${AUTOPUS_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/autopus"
GIT_DIR=${AUTOPUS_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${AUTOPUS_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${AUTOPUS_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${AUTOPUS_VERBOSE:-0}"
VERIFY_INSTALL="${AUTOPUS_VERIFY_INSTALL:-0}"
AUTOPUS_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Autopus installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://autopus.ai/install.sh | bash -s -- [options]
...
Environment variables:
  ...
  AUTOPUS_VERSION=latest|next|main|<semver>|<spec>
  AUTOPUS_BETA=0|1
  AUTOPUS_GIT_DIR=...
  AUTOPUS_GIT_UPDATE=0|1
  AUTOPUS_NO_PROMPT=1
  AUTOPUS_VERIFY_INSTALL=1
  AUTOPUS_DRY_RUN=1
...
Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://autopus.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://autopus.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://autopus.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://autopus.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://autopus.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Autopus checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Autopus source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_autopus_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"autopus"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://autopus.ai/install.sh | bash"
}
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future Autopus updates; use npm i -g autopus@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_autopus_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/autopus" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/autopus" ]]; then
        ln -sf "$npm_root/autopus/dist/entry.js" "${npm_bin}/autopus"
        ui_info "Created autopus bin link at ${npm_bin}/autopus"
    fi
    return 0
}

# Check for existing Autopus installation
check_existing_autopus() {
    if [[ -n "$(type -P autopus 2>/dev/null || true)" ]]; then
        ui_info "Existing Autopus installation detected, upgrading"
        return 0
    fi
    return 1
}
...
resolve_git_autopus_ref() {
    ...
    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "autopus" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_autopus_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${AUTOPUS_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_autopus_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/autopus"
        ...
        local version=""
        version="$(autopus_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_autopus_npm_root_candidates)
}

warn_duplicate_autopus_global_installs() {
    local installs=()
    local line=""
    while IFS= read -r line; do
        [[ -n "$line" ]] && installs+=("$line")
    done < <(find_autopus_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple Autopus global installs detected"
    echo "  Different Node/npm environments can run different Autopus versions."

    local active_node active_npm active_autopus
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_autopus="${AUTOPUS_BIN:-}"
    if [[ -z "$active_autopus" ]]; then
        active_autopus="$(type -P autopus 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active autopus: ${INFO}${active_autopus:-none}${NC}"
    echo ""
    echo "  Found installs:"
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make autopus show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

autopus_command_for_user() {
    local claw="${1:-}"
    if [[ -z "$claw" ]]; then
        echo "autopus"
        return 0
...
warn_autopus_not_found() {
    ui_warn "Installed, but autopus is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t autopus 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named autopus; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_autopus_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P autopus 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/autopus" ]]; then
        echo "${npm_bin}/autopus"
        return 0
    fi
...
install_autopus_from_git() {
    ...
    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Autopus from git checkout: ${repo_dir}"
    else
        ui_info "Installing Autopus from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Autopus" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Autopus" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/autopus" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/autopus"
    ui_success "Autopus wrapper installed to \$HOME/.local/bin/autopus"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view autopus dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:autopus/autopus#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_autopus() {
    local package_name="autopus"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            AUTOPUS_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="autopus"
        else
            AUTOPUS_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${AUTOPUS_VERSION}" ]]; then
        AUTOPUS_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${AUTOPUS_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${AUTOPUS_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Autopus v${resolved_version}"
    else
        ui_info "Installing Autopus (${AUTOPUS_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${AUTOPUS_VERSION}")"

    if ! install_autopus_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_autopus_paths
        install_autopus_npm "${install_spec}"
    fi

    if [[ "${AUTOPUS_VERSION}" == "latest" && "${package_name}" == "autopus" ]]; then
        if ! resolve_autopus_bin &> /dev/null; then
            ui_warn "npm install autopus@latest failed; retrying autopus@next"
            cleanup_npm_autopus_paths
            install_autopus_npm "autopus@next"
        fi
    fi

    ensure_autopus_bin_link || true

    ui_success "Autopus installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${AUTOPUS_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_autopus_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (autopus not on PATH yet)"
        warn_autopus_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${AUTOPUS_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.autopus/workspace-${profile}"
    else
        echo "${HOME}/.autopus/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${AUTOPUS_CONFIG_PATH:-$HOME/.autopus/autopus.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        local user_claw
        user_claw="$(autopus_command_for_user "${AUTOPUS_BIN:-}")"
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but autopus not on PATH; skipping onboarding"
        warn_autopus_not_found
        return
    fi
    ...
    "$claw" onboard || {
        local user_claw
        user_claw="$(autopus_command_for_user "$claw")"
...
if ! declare -F extract_autopus_semver >/dev/null 2>&1; then
...
resolve_autopus_version() {
    ...
    local claw="${AUTOPUS_BIN:-}"
    if [[ -z "$claw" ]] && command -v autopus &> /dev/null; then
        claw="$(command -v autopus)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_autopus_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/autopus/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/autopus/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: autopus not on PATH yet"
        warn_autopus_not_found
        return 1
    fi

    run_quiet_step "Checking Autopus version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: autopus gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_autopus_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Autopus checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set AUTOPUS_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_autopus; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Autopus"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g autopus &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g autopus 2>/dev/null || true
        ...
        install_autopus_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/autopus" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/autopus"
    ...
    AUTOPUS_BIN="$(resolve_autopus_bin || true)"
    warn_duplicate_autopus_global_installs || true
    ...
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🐙 Autopus installed successfully (${installed_version})!"
    else
        ui_celebrate "🐙 Autopus installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/autopus"
        ui_kv "Update command" "autopus update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://autopus.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running autopus doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                AUTOPUS_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                AUTOPUS_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                AUTOPUS_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
        ...
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (autopus not on PATH yet)"
                    warn_autopus_not_found
                    return 0
                fi
                exec </dev/tty
    ...
    if command -v autopus &> /dev/null; then
        ...
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (autopus daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if AUTOPUS_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: autopus daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${AUTOPUS_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
