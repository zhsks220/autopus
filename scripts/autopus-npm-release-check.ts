...
const EXPECTED_REPOSITORY_URL = "https://github.com/autopus/autopus";
...
const skipPackValidationEnv = "AUTOPUS_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "autopus") {
    errors.push(`package.json name must be "autopus"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.autopus !== "autopus.mjs") {
    errors.push(
      `package.json bin.autopus must be "autopus.mjs"; found "${pkg.bin?.autopus ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`autopus-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `autopus-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
