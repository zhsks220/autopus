...
elif [[ -n "${AUTOPUS_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(autopus_live_collect_auth_dirs_from_csv "${AUTOPUS_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(autopus_live_collect_auth_files_from_csv "${AUTOPUS_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
DOCKER_RUN_ARGS=(docker run --rm -t \
  ...
  -e AUTOPUS_LIVE_GATEWAY_MODELS="${AUTOPUS_LIVE_GATEWAY_MODELS:-modern}" \
  -e AUTOPUS_LIVE_GATEWAY_PROVIDERS="${AUTOPUS_LIVE_GATEWAY_PROVIDERS:-}" \
  -e AUTOPUS_LIVE_GATEWAY_SMOKE="${AUTOPUS_LIVE_GATEWAY_SMOKE:-1}" \
  -e AUTOPUS_LIVE_GATEWAY_MAX_MODELS="${AUTOPUS_LIVE_GATEWAY_MAX_MODELS:-8}" \
  -e AUTOPUS_LIVE_GATEWAY_HEARTBEAT_MS="${AUTOPUS_LIVE_GATEWAY_HEARTBEAT_MS:-}" \
  -e AUTOPUS_LIVE_GATEWAY_STEP_TIMEOUT_MS="${AUTOPUS_LIVE_GATEWAY_STEP_TIMEOUT_MS:-45000}" \
  -e AUTOPUS_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${AUTOPUS_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-90000}" \
