---
name: autopus-ghsa-maintainer
description: Inspect, patch, validate, publish, or confirm Autopus GHSA security advisories and private-fork state.
---

# Autopus GHSA Maintainer

Use this skill for repo security advisory workflow only. Keep general release work in `autopus-release-maintainer`.
...
```bash
fork=$(gh api /repos/autopus/autopus/security-advisories/<GHSA> | jq -r .private_fork.full_name)
...
```bash
gh api -X PATCH /repos/autopus/autopus/security-advisories/<GHSA> \
