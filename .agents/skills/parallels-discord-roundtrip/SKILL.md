...
```bash
export AUTOPUS_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.autopus/autopus.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env AUTOPUS_PARALLELS_DISCORD_TOKEN \
...
- Use the `autopus` wrapper for guest `message send/read`; `node autopus.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/autopus-parallels-smoke.*`
...
- host inbound nonce appears in `autopus message read` output
