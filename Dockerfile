...
ARG AUTOPUS_EXTENSIONS=""
ARG AUTOPUS_BUNDLED_PLUGIN_DIR=extensions
...
ARG AUTOPUS_NODE_BOOKWORM_SLIM_DIGEST="sha256:e8e2e91b1378f83c5b2dd15f0247f34110e2fe895f6ca7719dbb780f929368eb"
# Keep in sync with .github/actions/setup-node-env/action.yml bun-version.
# To update: docker buildx imagetools inspect oven/bun:<version> and use the manifest-list digest.
ARG AUTOPUS_BUN_IMAGE="oven/bun:1.3.13@sha256:87416c977a612a204eb54ab9f3927023c2a3c971f4f345a01da08ea6262ae30e"
...
FROM ${AUTOPUS_NODE_BOOKWORM_IMAGE} AS workspace-deps
ARG AUTOPUS_EXTENSIONS
ARG AUTOPUS_BUNDLED_PLUGIN_DIR
...
RUN --mount=type=bind,source=packages,target=/tmp/packages,readonly \
    --mount=type=bind,source=${AUTOPUS_BUNDLED_PLUGIN_DIR},target=/tmp/${AUTOPUS_BUNDLED_PLUGIN_DIR},readonly \
    mkdir -p /out/packages "/out/${AUTOPUS_BUNDLED_PLUGIN_DIR}" && \
    ...
    for ext in $(printf '%s\n' "$AUTOPUS_EXTENSIONS" | tr ',' ' '); do \
      if [ -f "/tmp/${AUTOPUS_BUNDLED_PLUGIN_DIR}/$ext/package.json" ]; then \
        mkdir -p "/out/${AUTOPUS_BUNDLED_PLUGIN_DIR}/$ext" && \
        cp "/tmp/${AUTOPUS_BUNDLED_PLUGIN_DIR}/$ext/package.json" "/out/${AUTOPUS_BUNDLED_PLUGIN_DIR}/$ext/package.json"; \
      fi; \
    done

# ── Stage 2: Build ──────────────────────────────────────────────
FROM ${AUTOPUS_BUN_IMAGE} AS bun-binary
FROM ${AUTOPUS_NODE_BOOKWORM_IMAGE} AS build
ARG AUTOPUS_BUNDLED_PLUGIN_DIR
...
ARG AUTOPUS_NODE_BOOKWORM_SLIM_DIGEST
LABEL org.opencontainers.image.base.name="docker.io/library/node:24-bookworm-slim" \
  org.opencontainers.image.base.digest="${AUTOPUS_NODE_BOOKWORM_SLIM_DIGEST}"
...
# - https://docs.autopus.ai/install/docker
LABEL org.opencontainers.image.source="https://github.com/autopus/autopus" \
  org.opencontainers.image.url="https://autopus.ai" \
  org.opencontainers.image.documentation="https://docs.autopus.ai/install/docker" \
  org.opencontainers.image.licenses="MIT" \
  org.opencontainers.image.title="Autopus" \
  org.opencontainers.image.description="Autopus gateway and CLI runtime container image"
...
RUN --mount=type=cache,id=autopus-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=autopus-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    apt-get update && \
...
COPY --from=runtime-assets --chown=node:node /app/autopus.mjs .
COPY --from=runtime-assets --chown=node:node /app/${AUTOPUS_BUNDLED_PLUGIN_DIR} ./${AUTOPUS_BUNDLED_PLUGIN_DIR}
...
# Example: docker build --build-arg AUTOPUS_DOCKER_APT_PACKAGES="python3 wget" .
ARG AUTOPUS_DOCKER_APT_PACKAGES=""
RUN --mount=type=cache,id=autopus-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=autopus-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$AUTOPUS_DOCKER_APT_PACKAGES" ]; then \
      apt-get update && \
      DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends $AUTOPUS_DOCKER_APT_PACKAGES; \
    fi

# Optionally install Chromium and Xvfb for browser automation.
# Build with: docker build --build-arg AUTOPUS_INSTALL_BROWSER=1 ...
# Adds ~300MB but eliminates the 60-90s Playwright install on every container start.
# Must run after node_modules COPY so playwright-core is available.
ARG AUTOPUS_INSTALL_BROWSER=""
...
# Build with: docker build --build-arg AUTOPUS_INSTALL_DOCKER_CLI=1 ...
# Adds ~50MB. Only the CLI is installed — no Docker daemon.
# Required for agents.defaults.sandbox to function in Docker deployments.
ARG AUTOPUS_INSTALL_DOCKER_CLI=""
ARG AUTOPUS_DOCKER_GPG_FINGERPRINT="9DC858229FC7DD38854AE2D88D81803C0EBFCD88"
RUN --mount=type=cache,id=autopus-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=autopus-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$AUTOPUS_INSTALL_DOCKER_CLI" ]; then \
      apt-get update && \
      DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
      ...
      # Update AUTOPUS_DOCKER_GPG_FINGERPRINT when Docker rotates release keys.
      curl -fsSL https://download.docker.com/linux/debian/gpg -o /tmp/docker.gpg.asc && \
      expected_fingerprint="$(printf '%s' "$AUTOPUS_DOCKER_GPG_FINGERPRINT" | tr '[:lower:]' '[:upper:]' | tr -d '[:space:]')" && \
...
RUN ln -sf /app/autopus.mjs /usr/local/bin/autopus \
 && chmod 755 /app/autopus.mjs
...
ENTRYPOINT ["tini", "-s", "--"]
CMD ["node", "autopus.mjs", "gateway", "--allow-unconfigured"]
