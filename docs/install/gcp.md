---
summary: "Run Autopus Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Autopus running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent Autopus Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Autopus 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-autopus-project --name="Autopus Gateway"
    gcloud config set project my-autopus-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create autopus-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh autopus-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Configure environment variables">
    ...
    AUTOPUS_CONFIG_DIR=/home/$USER/.autopus
    AUTOPUS_WORKSPACE_DIR=/home/$USER/.autopus/workspace
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`AUTOPUS_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm autopus-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh autopus-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop autopus-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type autopus-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start autopus-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create autopus-deploy \
     --display-name="Autopus Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-autopus-project \
     --member="serviceAccount:autopus-deploy@my-autopus-project.iam.gserviceaccount.com" \
