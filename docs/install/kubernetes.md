---
summary: "Deploy Autopus Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Autopus on a Kubernetes cluster
  - You want to test Autopus in a Kubernetes environment
title: "Kubernetes"
---

A minimal starting point for running Autopus on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.
...
kubectl port-forward svc/autopus 18789:18789 -n autopus
...
```bash
kubectl get secret autopus-secrets -n autopus -o jsonpath='{.data.AUTOPUS_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: autopus (configurable via AUTOPUS_NAMESPACE)
├── Deployment/autopus        # Single pod, init container + gateway
├── Service/autopus           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/autopus-config  # autopus.json + AGENTS.md
└── Secret/autopus-secrets    # Gateway token + API keys
```
...
Edit `autopus.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret autopus-secrets -n autopus \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/autopus -n autopus
```

### Custom namespace

```bash
AUTOPUS_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/autopus/autopus:latest # or pin to a specific version from https://github.com/autopus/autopus/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # autopus.json + AGENTS.md
