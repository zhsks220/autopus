---
summary: "Deploy Autopus on Render with Infrastructure-as-Code"
read_when:
  - Deploying Autopus to Render
...
Deploy Autopus on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/autopus/autopus)
...
repository configures everything needed to run Autopus:
...
services:
  - type: web
    ...
    envVars:
      - key: AUTOPUS_GATEWAY_PORT
        value: "8080"
      - key: AUTOPUS_STATE_DIR
        value: /data/.autopus
      - key: AUTOPUS_WORKSPACE_DIR
        value: /data/workspace
      - key: AUTOPUS_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means Autopus state
...
`AUTOPUS_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original Autopus repository, Render will not auto-deploy your Autopus. To update it, run a manual Blueprint sync from the dashboard.
...
For Autopus, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
- Missing `AUTOPUS_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `AUTOPUS_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `autopus backup create` in the Render shell.
