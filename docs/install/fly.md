---
summary: "Step-by-step Fly.io deployment for Autopus with persistent storage and HTTPS"
title: Fly.io
read_when:
  - Deploying Autopus on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

**Goal:** Autopus Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
<Steps>
  <Step title="Create the Fly app">
    ...
    fly apps create my-autopus

    # Create a persistent volume (1GB is usually enough)
    fly volumes create autopus_data --size 1 --region iad
    ```
  ...
  <Step title="Configure fly.toml">
    ...
    **Security note:** The default config exposes a public URL. For a hardened deployment with no public IP, see [Private Deployment](#private-deployment-hardened) or use `deploy/fly.private.toml`.
    ...
    [env]
      NODE_ENV = "production"
      AUTOPUS_PREFER_PNPM = "1"
      AUTOPUS_STATE_DIR = "/data"
    ...
    The Autopus Docker image uses `tini` as its entrypoint. Fly process commands replace Docker `CMD` without replacing `ENTRYPOINT`, so the process still runs under `tini`.
    ...
    **Key settings:**
    ...
    | `internal_port = 3000`         | Must match `--port 3000` (or `AUTOPUS_GATEWAY_PORT`) for Fly health checks |
    | `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
    | `AUTOPUS_STATE_DIR = "/data"` | Persists state on the volume                                                |
  ...
  <Step title="Set secrets">
    ...
    fly secrets set AUTOPUS_GATEWAY_TOKEN=$(openssl rand -hex 32)
    ...
    **Notes:**
    ...
    - Non-loopback binds (`--bind lan`) require a valid gateway auth path. This Fly.io example uses `AUTOPUS_GATEWAY_TOKEN`, but `gateway.auth.password` or a correctly configured non-loopback `trusted-proxy` deployment also satisfy the requirement.
    - Treat these tokens like passwords.
    - **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `autopus.json` where they could be accidentally exposed or logged.
  ...
  <Step title="Create config file">
    ...
    **Note:** With `AUTOPUS_STATE_DIR=/data`, the config path is `/data/autopus.json`.

    **Note:** Replace `https://my-autopus.fly.dev` with your real Fly app
  ...
  <Step title="Access the Gateway">
    ...
    Or visit `https://my-autopus.fly.dev/`

    Authenticate with the configured shared secret. This guide uses the gateway
    token from `AUTOPUS_GATEWAY_TOKEN`; if you switched to password auth, use
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `AUTOPUS_GATEWAY_PORT=3000`).
...
`--allow-unconfigured` only bypasses the startup guard. It does not create or repair `/data/autopus.json`, so make sure your real config exists and includes `gateway.mode="local"` when you want a normal local gateway start.

Verify the config exists:

```bash
fly ssh console --command "cat /data/autopus.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/autopus.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/autopus.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/autopus.json"
```
...
**Fix:** Ensure `AUTOPUS_STATE_DIR=/data` is set in `fly.toml` and redeploy.
...
```bash
# List current IPs
fly ips list -a my-autopus

# Release public IPs
fly ips release <public-ipv4> -a my-autopus
fly ips release <public-ipv6> -a my-autopus
...
fly ips allocate-v6 --private -a my-autopus
```
...
fly proxy 3000:3000 -a my-autopus
