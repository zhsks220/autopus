---
summary: "Install and use Codex, Claude, and Cursor bundles as Autopus plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Autopus maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

Autopus can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
Autopus maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Autopus plugins. Native plugins run
...
of requiring authors to rewrite them as native Autopus plugins, Autopus
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    autopus plugins install ./my-bundle

    # Archive
    autopus plugins install ./my-bundle.tgz
...
## What Autopus maps from bundles

Not every bundle feature runs in Autopus today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Autopus skills                                            | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                  | Claude, Cursor |
| Hook packs    | Autopus-style `HOOK.md` + `handler.ts` layouts                                              | Codex          |
...
- bundle skill roots load as normal Autopus skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal Autopus skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal Autopus hook-pack
...
- Autopus merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- Autopus exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, Autopus uses `sse`
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in Autopus config. `autopus mcp set` and `autopus doctor --fix` normalize the common alias.
...
Autopus registers bundle MCP tools with provider-safe names in the form
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- Autopus sanitizes shell override keys before applying them
...
- Autopus loads `.lsp.json` plus any manifest-declared `lspServers` paths
...
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `autopus plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but Autopus does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Autopus best when they use skill roots and Autopus-style
...
Autopus checks for native plugin format first:

1. `autopus.plugin.json` or valid `package.json` with `autopus.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, Autopus uses the native path. This prevents
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `autopus plugins install` and ship everything
  they need in the installed plugin directory.
- Autopus-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `autopus doctor --fix` removes legacy staged dependency directories and can
...
- Autopus does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `autopus plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. Autopus does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Autopus hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
