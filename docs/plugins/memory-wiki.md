...
If the active memory plugin exposes shared recall artifacts, Autopus can search
...
currently exposing public bridge inputs yet. Run `autopus wiki doctor` first,
...
`autopus wiki status`, `autopus wiki doctor`, and `autopus wiki bridge
...
autopus wiki apply synthesis "Alpha Summary" --body "..." --source-id source.alpha
