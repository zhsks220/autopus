...
```bash
autopus nodes describe --node <idOrNameOrIp>
autopus nodes canvas snapshot --node <idOrNameOrIp>
autopus logs --follow
```
...
autopus nodes status
autopus approvals get --node <idOrNameOrIp>
autopus approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`autopus approvals get --node ...`), not in the gateway pairing record.
