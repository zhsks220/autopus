...
Autopus supports **camera capture** for agent workflows:
...
```bash
autopus nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
autopus nodes camera snap --node <id> --facing front
autopus nodes camera clip --node <id> --duration 3000
autopus nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`autopus.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return "Camera disabled by user".

### CLI helper (node invoke)

Use the main `autopus` CLI to invoke camera commands on the macOS node.

Examples:

```bash
autopus nodes camera list --node <id>            # list camera ids
autopus nodes camera snap --node <id>            # prints MEDIA:<path>
autopus nodes camera snap --node <id> --max-width 1280
autopus nodes camera snap --node <id> --delay-ms 2000
autopus nodes camera snap --node <id> --device-id <id>
autopus nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
autopus nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
autopus nodes camera clip --node <id> --device-id <id>
autopus nodes camera clip --node <id> --no-audio
```

Notes:

- `autopus nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
autopus nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
