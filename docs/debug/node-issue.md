...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Autopus
...
Running Autopus via Node with `tsx` fails at startup with:

```
[autopus] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `autopus status` and `gateway:watch` worked.
