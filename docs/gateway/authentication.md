...
Autopus supports OAuth and API keys for model providers. For always-on gateway
...
setup, but Autopus also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `autopus gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.autopus/.env` so the daemon can read it:
...
API keys for daemon use: `autopus onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.autopus/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in Autopus as a supported token
path. Anthropic staff has since told us that Autopus-style Claude CLI usage is
allowed again, so Autopus treats Claude CLI reuse and `claude -p` usage as
...
2. Tell Autopus to switch Anthropic model selection to the local `claude-cli`
   backend and store the matching Autopus auth profile.
...
```bash
autopus models auth paste-token --provider openrouter
```
...
- If auth exists but Autopus cannot resolve a probeable model candidate for
...
- Anthropic staff told us this Autopus integration path is allowed again.
- Autopus therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `AUTOPUS_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Autopus retries with the next key only for rate-limit errors (for example
...
When you debug order issues, `autopus models status --probe` shows omitted
...
Run `autopus models status` to confirm which profile is expiring. If an
