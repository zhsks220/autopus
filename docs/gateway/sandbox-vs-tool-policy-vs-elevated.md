...
Autopus has three related (but different) controls:
...
Use the inspector to see what Autopus is _actually_ doing:
...
- Autopus validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:autopus`: all built-in Autopus tools (excludes provider plugins)
