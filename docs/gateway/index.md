...
Healthy baseline: `Runtime: running`, `Connectivity probe: ok`, and `Capability: ...` that matches what you expect. Use `autopus gateway status --require-rpc` when you need read-scope RPC proof, not just reachability.
...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `AUTOPUS_CONFIG_PATH` when set).
...
- Auth is required by default. Shared-secret setups use
  ...
  `AUTOPUS_GATEWAY_TOKEN` / `AUTOPUS_GATEWAY_PASSWORD`), and non-loopback
  reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

Autopus's highest-leverage compatibility surface is now:
...
- `/v1/models` is agent-first: it returns `autopus`, `autopus/default`, and `autopus/<agentId>`.
- `autopus/default` is the stable alias that always maps to the configured default agent.
- Use `x-autopus-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | ------------------------------------------------------------ |
| Gateway port | `--port` → `AUTOPUS_GATEWAY_PORT` → `gateway.port` → `18789` |
| Bind mode    | CLI/override → `gateway.bind` → `loopback`                   |

Installed gateway services record the resolved `--port` in supervisor metadata. After changing `gateway.port`, run `autopus doctor --fix` or `autopus gateway install --force` so launchd/systemd/schtasks starts the process on the new port.
...
```bash
autopus gateway status
autopus gateway status --deep   # adds a system-level service scan
...
- Unique `gateway.port`
- Unique `AUTOPUS_CONFIG_PATH`
- Unique `AUTOPUS_STATE_DIR`
- Unique `agents.defaults.workspace`

Example:

```bash
AUTOPUS_CONFIG_PATH=~/.autopus/a.json AUTOPUS_STATE_DIR=~/.autopus-a autopus gateway --port 19001
AUTOPUS_CONFIG_PATH=~/.autopus/b.json AUTOPUS_STATE_DIR=~/.autopus-b autopus gateway --port 19002
```
...
Use `autopus gateway restart` for restarts. Do not chain `autopus gateway stop` and `autopus gateway start` as a restart substitute.

On macOS, `gateway stop` uses `launchctl bootout` by default — this removes the LaunchAgent from the current boot session without persisting a disable, so KeepAlive auto-recovery still works after unexpected crashes and `gateway start` re-enables cleanly. To persistently suppress auto-respawn across reboots, pass `--disable`: `autopus gateway stop --disable`.

LaunchAgent labels are `ai.autopus.gateway` (default) or `ai.autopus.<profile>` (named profile). `autopus doctor` audits and repairs service config drift.
...
Native Windows managed startup uses a Scheduled Task named `Autopus Gateway`
(or `Autopus Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, Autopus falls back to a per-user Startup-folder launcher
...
sudo systemctl enable --now autopus-gateway[-<profile>].service
```

Use the same service body as the user unit, but install it under
`/etc/systemd/system/autopus-gateway[-<profile>].service` and adjust
`ExecStart=` if your `autopus` binary lives elsewhere.

Do not also let `autopus doctor --fix` install a user-level gateway service for the same profile/port. Doctor refuses that automatic install when it finds a system-level Autopus gateway service; use `AUTOPUS_SERVICE_REPAIR_POLICY=external` when the system unit owns the lifecycle.
...
```bash
autopus --dev setup
autopus --dev gateway --allow-unconfigured
autopus --dev status
```
