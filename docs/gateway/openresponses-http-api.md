...
Autopus's Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`autopus agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-autopus-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-autopus-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "autopus"`, `model: "autopus/default"`, `model: "autopus/<agentId>"`, or `x-autopus-agent-id`
- use `x-autopus-model` when you want to override the selected agent's backend model
- use `x-autopus-session-key` for explicit session routing
- use `x-autopus-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: Autopus reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
Autopus normalizes common OpenAI-style aliases before those counters reach
