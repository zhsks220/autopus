...
service via the external `autopus` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.autopus.gateway`
  (or `ai.autopus.<profile>` when using `--profile`/`AUTOPUS_PROFILE`; legacy `com.autopus.*` is supported).
