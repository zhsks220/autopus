...
- Same path: run the app from a fixed location (for Autopus, `dist/Autopus.app`).
...
```bash
sudo tccutil reset Accessibility ai.autopus.mac
sudo tccutil reset ScreenCapture ai.autopus.mac
...
Workaround: move files into the Autopus workspace (`~/.autopus/workspace`) if you want to avoid per-folder grants.
