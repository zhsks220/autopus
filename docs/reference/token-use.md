---
summary: "How Autopus builds prompt context and reports token usage + costs"
read_when:
...
Autopus tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Autopus assembles its own system prompt on every run. It includes:
...
For images, Autopus downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Autopus session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `autopus status --usage` and `autopus channels list` show
...
available; otherwise Autopus falls back to matching OAuth/API-key credentials
...
Autopus keeps provider usage accounting separate from the current context
...
`cacheWrite`. If pricing is missing, Autopus shows tokens only. OAuth tokens
never show dollar cost.

After sidecars and channels reach the Gateway ready path, Autopus starts an
...
Provider prompt caching only applies within the cache TTL window. Autopus can
...
Anthropic's 1M context window is currently beta-gated. Autopus can inject the
...
Autopus skips the `context-1m-*` beta header because Anthropic currently
