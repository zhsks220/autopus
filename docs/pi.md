---
summary: "Architecture of Autopus's embedded Pi agent integration and session lifecycle"
title: "Pi integration architecture"
read_when:
  - Understanding Pi SDK integration design in Autopus
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

Autopus integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Autopus uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Autopus directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Autopus's local TUI mode)                                              |
...
├── pi-tools.ts                    # createAutopusCodingTools()
...
├── autopus-tools.ts              # Autopus-specific tools
...
Image injection is prompt-local: Autopus loads image refs from the current prompt and
...
2. **Custom Replacements**: Autopus replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Autopus Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
Autopus wraps this with `guardSessionManager()` for tool result safety.
...
Autopus maintains an auth profile store with multiple API keys per provider:
...
Autopus loads custom pi extensions for specialized behavior:
...
Autopus also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Autopus Embedded                                                                             |
| --------------- | ----------------------- | -------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                               |
| Tools           | Default coding tools    | Custom Autopus tool suite                                                                    |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                  |
| Session storage | `~/.pi/agent/sessions/` | `~/.autopus/agents/<agentId>/sessions/` (or `$AUTOPUS_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `AUTOPUS_LIVE_TEST=1`)
