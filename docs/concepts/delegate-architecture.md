---
summary: "Delegate architecture: running Autopus as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run Autopus as a **named delegate** - an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Autopus agent that:
...
Autopus's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Autopus's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.autopus/workspace-delegate",
  tools: {
...
- Cron run history: `~/.autopus/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.autopus/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Autopus's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
autopus agents add delegate
```

This creates:

- Workspace: `~/.autopus/workspace-delegate`
- State: `~/.autopus/agents/delegate/agent`
- Sessions: `~/.autopus/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.autopus/workspace" },
      {
        id: "delegate",
        workspace: "~/.autopus/workspace-delegate",
        tools: {
...
~/.autopus/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.autopus/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.autopus/workspace-org",
        agentDir: "~/.autopus/agents/org-assistant/agent",
...
recall view. Autopus redacts credential/token-like text, truncates long
