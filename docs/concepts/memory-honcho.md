...
[Honcho](https://honcho.dev) adds AI-native memory to Autopus. It persists
...
```bash
autopus plugins install @honcho-ai/autopus-honcho
autopus honcho setup
...
Settings live under `plugins.entries["autopus-honcho"].config`:
    ...
    entries: {
      "autopus-honcho": {
        config: {
          ...
          workspaceId: "autopus", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `autopus honcho setup` detects and
...
```bash
autopus honcho setup                        # Configure API key and migrate files
autopus honcho status                       # Check connection status
autopus honcho ask <question>               # Query Honcho about the user
autopus honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/autopus-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Autopus integration guide](https://docs.honcho.dev/v3/guides/integrations/autopus)
- [Memory](/concepts/memory) -- Autopus memory overview
