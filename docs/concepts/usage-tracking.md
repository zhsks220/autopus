...
- `/usage cost` in chats: local cost summary aggregated from Autopus session logs.
- CLI: `autopus status --usage` prints a full per-provider breakdown.
- CLI: `autopus channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Autopus treats
  ...
  quota, so Autopus inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Autopus prefers the
...
can supply plugin-specific usage auth logic; otherwise Autopus falls back to
