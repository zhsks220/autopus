---
summary: "Use SOUL.md to give your Autopus agent an actual voice instead of generic assistant sludge"
read_when:
...
Autopus injects it on normal sessions, so it has real weight. If your agent
...
For Autopus, `SOUL.md` is that layer.
...
Path fixed for Autopus workspaces: use `SOUL.md`, not `http://SOUL.md`.
...
<CardGroup cols={2}>
  <Card title="Agent workspace" href="/concepts/agent-workspace" icon="folder-open">
    Workspace files Autopus injects into the system prompt.
  </Card>
