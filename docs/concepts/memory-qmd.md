...
alongside Autopus. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, Autopus falls back to the
...
Autopus creates a self-contained QMD home under
`~/.autopus/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- Autopus creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so Autopus skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, Autopus retries with `qmd query`.
- With QMD releases that advertise multi-collection filters, Autopus groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, Autopus falls back to the builtin SQLite engine.
  ...
  `autopus memory status` and one-shot CLI probes still recheck QMD directly.
...
Autopus keeps the QMD search path compatible with both current and older QMD
installs.

On startup, Autopus checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, Autopus searches all
...
Older QMD builds only accept one collection filter. When Autopus detects one
...
process, so you can tune QMD globally without adding new Autopus config:
...
collection under `~/.autopus/agents/<id>/qmd/sessions/`.
...
When scope denies a search, Autopus logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If Autopus
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but Autopus still reports
...
with `autopus memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs Autopus uses.

**Many QMD subprocesses during search?** Update QMD if possible. Autopus uses
...
`memory.qmd.searchMode = "search"`. Autopus treats that mode as lexical-only,
...
the next startup reconciliation. Autopus recreates stale managed collections
...
Autopus's builtin symlink rules. Keep temporary monorepo checkouts under
