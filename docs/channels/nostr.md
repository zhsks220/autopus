...
read_when:
  - You want Autopus to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Autopus to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current Autopus releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`autopus onboard`) and `autopus channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install the npm package directly.

```bash
autopus plugins install @autopus/nostr
```
...
```bash
autopus plugins install --link <path-to-local-nostr-plugin>
```
...
autopus channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "autopus@example.com",
        lud16: "autopus@example.com",
      },
    },
  },
}
