...
read_when:
  - You want to connect Autopus to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to Autopus via the LINE Messaging API. The plugin runs as a webhook
...
```bash
autopus plugins install @autopus/line
```

Local checkout (when running from a git repo):

```bash
autopus plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Autopus applies strict pre-auth body limits and timeout before verification.
- Autopus processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
autopus pairing list line
autopus pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.autopus/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. Autopus validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
