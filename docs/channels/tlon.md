...
Tlon is a decentralized messenger built on Urbit. Autopus connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Autopus releases, so normal packaged
...
```bash
autopus plugins install @autopus/tlon
```
...
```bash
autopus plugins install ./path/to/local/tlon-plugin
```
...
By default, Autopus blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `autopus message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Autopus replies in-thread.
