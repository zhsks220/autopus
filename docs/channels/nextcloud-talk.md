...
Nextcloud Talk ships as a bundled plugin in current Autopus releases, so
...
```bash
autopus plugins install @autopus/nextcloud-talk
```
...
```bash
autopus plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Autopus" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure Autopus:
   ...
   ```bash
   autopus channels add --channel nextcloud-talk \
...
- Approve via:
  - `autopus pairing list nextcloud-talk`
  - `autopus pairing approve nextcloud-talk <CODE>`
