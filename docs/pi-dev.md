...
A sane workflow for working on the Pi integration in Autopus.
...
```bash
AUTOPUS_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm autopus agent --message "Hello" --thinking low`
...
State lives under the Autopus state directory. Default is `~/.autopus`. If `AUTOPUS_STATE_DIR` is set, use that directory instead.

To reset everything:

- `autopus.json` for config
