---
summary: "Use Kilo Gateway's unified API to access many models in Autopus"
title: "Kilo Gateway"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in Autopus
---
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    autopus onboard --auth-choice kilocode-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    autopus models list --provider kilocode
    ```
  </Step>
</Steps>
...
<Note>
Autopus treats `kilocode/kilo/auto` as the stable default ref, but does not
...
hard-coded in Autopus.
</Note>

## Built-in catalog

Autopus dynamically discovers available models from the Kilo Gateway at startup. Use
...
<Tip>
At startup, Autopus queries `GET https://api.kilo.ai/api/gateway/models` and merges
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path, so Autopus keeps
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails at startup, Autopus falls back to the bundled static catalog containing `kilocode/kilo/auto`.
    - Confirm your API key is valid and that your Kilo account has the desired models enabled.
    - When the Gateway runs as a daemon, ensure `KILOCODE_API_KEY` is available to that process (for example in `~/.autopus/.env` or via `env.shellEnv`).
