---
summary: "Use OpenCode Zen and Go catalogs with Autopus"
read_when:
...
OpenCode exposes two hosted catalogs in Autopus:
...
Both catalogs use the same OpenCode API key. Autopus keeps the runtime provider ids
    ...
    **Best for:** the curated OpenCode multi-model proxy (Claude, GPT, Gemini).
    ...
    <Steps>
      <Step title="Run onboarding">
        ...
        ```bash
        autopus onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
        ```
      </Step>
      <Step title="Set a Zen model as the default">
        ```bash
        autopus config set agents.defaults.model.primary "opencode/claude-opus-4-6"
        ```
      </Step>
      <Step title="Verify models are available">
        ```bash
        autopus models list --provider opencode
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Gemini replay behavior">
    Gemini-backed OpenCode refs stay on the proxy-Gemini path, so Autopus keeps
