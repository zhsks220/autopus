---
summary: "Use StepFun models with Autopus"
read_when:
  - You want StepFun models in Autopus
  - You need StepFun setup guidance
title: "StepFun"
---

Autopus includes a bundled StepFun provider plugin with two provider ids:
    ...
    **Best for:** general-purpose use via the standard StepFun endpoint.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        autopus onboard --auth-choice stepfun-standard-api-key-intl
        ```

        Or for the China endpoint:

        ```bash
        autopus onboard --auth-choice stepfun-standard-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        autopus models list --provider stepfun
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Step Plan">
    **Best for:** Step Plan reasoning endpoint.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        autopus onboard --auth-choice stepfun-plan-api-key-intl
        ```

        Or for the China endpoint:

        ```bash
        autopus onboard --auth-choice stepfun-plan-api-key-cn
        ```
      </Step>
      <Step title="Non-interactive alternative">
        ```bash
      ...
      <Step title="Verify models are available">
        ```bash
        autopus models list --provider stepfun-plan
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Notes">
    - The provider is bundled with Autopus, so there is no separate plugin install step.
    ...
    - Use `autopus models list` and `autopus models set <provider/model>` to inspect or switch models.
