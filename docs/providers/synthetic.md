---
summary: "Use Synthetic's Anthropic-compatible API in Autopus"
read_when:
...
Autopus registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    autopus onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Autopus's Anthropic client appends `/v1` to the base URL automatically, so use
...
`autopus models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Autopus appends `/v1` automatically.
