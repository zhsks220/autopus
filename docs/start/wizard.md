...
CLI onboarding is the **recommended** way to set up Autopus on macOS,
...
`autopus dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`autopus configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.autopus/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `autopus doctor` first.
</Note>
...
- CLI command reference: [`autopus onboard`](/cli/onboard)
