---
summary: "CLI reference for `autopus memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
autopus memory status
autopus memory status --deep
autopus memory status --fix
autopus memory index --force
autopus memory search "meeting notes"
autopus memory search --query "deployment" --max-results 20
autopus memory promote --limit 10 --min-score 0.75
autopus memory promote --apply
autopus memory promote --json --min-recall-count 0 --min-unique-queries 0
...
autopus memory status --json
autopus memory status --deep --index
autopus memory status --deep --index --verbose
autopus memory status --agent main
autopus memory index --agent main --verbose
```
...
```bash
autopus memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `autopus cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
autopus memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
autopus memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
