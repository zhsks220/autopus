---
summary: "CLI reference for `autopus dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You're setting up split DNS for a custom discovery domain (example: autopus.internal)
title: "DNS"
---
...
```bash
autopus dns setup
autopus dns setup --domain autopus.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `autopus.internal`)
...
- zone file path
- current tailnet IPs
- recommended `autopus.json` discovery config
...
- If `--domain` is omitted, Autopus uses `discovery.wideArea.domain` from config.
