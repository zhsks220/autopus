...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Autopus Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`autopus acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Autopus channel
conversations instead of hosting an ACP harness session, use
[`autopus mcp serve`](/cli/mcp) instead.
...
- Autopus acts as an ACP server
- an IDE or ACP client connects to Autopus
- Autopus forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Autopus runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Autopus: use `autopus acp`
- Autopus should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include Autopus parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                                 |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Autopus gateway or agent instead.                                                                                                                                      |
...
# Remote Gateway
autopus acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
autopus acp --url wss://gateway-host:18789 --token-file ~/.autopus/gateway.token

# Attach to an existing session key
autopus acp --session agent:main:main

# Attach by label (must already exist)
autopus acp --session-label "support inbox"

# Reset the session key before the first prompt
autopus acp --session agent:main:main --reset-session
```
...
autopus acp client --server-args --url wss://gateway-host:18789 --token-file ~/.autopus/gateway.token

# Override the server command (default: autopus)
autopus acp client --server "node" --server-args autopus.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Autopus through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`autopus acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `autopus gateway call sessions.list` as the only ACP proof. That
...
it to drive an Autopus Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `autopus acp` over stdio.

Example config (persisted):

```bash
autopus config set gateway.remote.url wss://gateway-host:18789
autopus config set gateway.remote.token <token>
```
...
```bash
autopus acp --session agent:main:main
autopus acp --session agent:design:main
autopus acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Autopus plugin tools or selected
...
[Autopus tools MCP bridge](/tools/acp-agents-setup#autopus-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
Autopus bot over ACP, use `acpx` with its built-in `autopus` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx autopus` at `autopus acp`.
3. Target the Autopus session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Autopus ACP session
acpx autopus exec "Summarize the active Autopus session state."

# Persistent named session for follow-up turns
acpx autopus sessions ensure --name codex-bridge
acpx autopus -s codex-bridge --cwd /path/to/repo \
  "Ask my Autopus work agent for recent context relevant to this repo."
```

If you want `acpx autopus` to target a specific Gateway and session key every
time, override the `autopus` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "autopus": {
      "command": "env AUTOPUS_HIDE_BANNER=1 AUTOPUS_SUPPRESS_NOTES=1 autopus acp --url ws://127.0.0.1:18789 --token-file ~/.autopus/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Autopus checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env AUTOPUS_HIDE_BANNER=1 AUTOPUS_SUPPRESS_NOTES=1 node autopus.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Autopus agent without scraping a terminal.
...
In Zed, open the Agent panel and select "Autopus ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`AUTOPUS_GATEWAY_TOKEN`, `AUTOPUS_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`AUTOPUS_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `AUTOPUS_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `autopus acp client` sets `AUTOPUS_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `autopus`).
