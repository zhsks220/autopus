---
summary: "CLI reference for `autopus approvals` and `autopus exec-policy`"
read_when:
...
`autopus exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.autopus/exec-approvals.json`

Examples:

```bash
autopus exec-policy show
autopus exec-policy show --json

autopus exec-policy preset yolo
autopus exec-policy preset cautious --json

autopus exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `autopus exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `autopus approvals set --gateway`
or `autopus approvals set --node <id|name|ip>`.

## Common commands

```bash
autopus approvals get
autopus approvals get --node <id|name|ip>
autopus approvals get --gateway
```

`autopus approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
autopus approvals set --file ./exec-approvals.json
autopus approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
autopus approvals set --node <id|name|ip> --file ./exec-approvals.json
autopus approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested Autopus policy aligned, also set:
...
```bash
autopus approvals allowlist add "~/Projects/**/bin/rg"
autopus approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
autopus approvals allowlist add --agent "*" "/usr/bin/uname"

autopus approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `autopus nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.autopus/exec-approvals.json`.
